...
class IntelligentChunker:
    ...
    def _semantic_chunking(
        ...
        for section_idx, (section_text, section_title) in enumerate(sections):
            ...
            for para in paragraphs:
                ...
                if (
                    current_tokens + para_tokens > config.max_chunk_tokens
                    and current_chunk_parts
                ):
                    # Create chunk from accumulated paragraphs
                    chunk_text = "\n\n".join(current_chunk_parts)
                    chunks.append(
                        self._create_enhanced_chunk(
                            chunk_text,
                            len(chunks),
        ...
        for section_idx, (section_text, section_title) in enumerate(sections):
            ...
            if section_tokens <= config.max_chunk_tokens:
                ...
                sub_chunks = self._split_large_section(
                    section_text, section_title, config
                )
    ...
    def _sliding_window_chunking(
        ...
        while start < len(text):
            ...
            chunks.append(
                self._create_enhanced_chunk(
                    chunk_text, len(chunks), chunk_type="window"
                )
            )
    ...
    def _calculate_coherence(self, text: str) -> float:
        ...
        transition_count = sum(
            1 for word in self.TOPIC_TRANSITIONS if word in text.lower()
        )
        return min(1.0, 0.8 + (transition_count * 0.05))

    def _add_context_to_chunks(
        self, chunks: list[EnhancedTextChunk]
    ) -> list[EnhancedTextChunk]:
    ...
    def _split_large_section(
        self, text: str, section_title: str, config: ChunkingConfig
    ) -> list[EnhancedTextChunk]:
    ...
    def _refine_chunk_semantically(
        self, chunk: EnhancedTextChunk, config: ChunkingConfig
    ) -> list[EnhancedTextChunk]:
    ...
    def _merge_small_chunks(
        self, chunks: list[EnhancedTextChunk], config: ChunkingConfig
    ) -> list[EnhancedTextChunk]:
        ...
        for next_chunk in chunks[1:]:
            ...
            if (
                combined_tokens <= config.max_chunk_tokens * 0.9
                and current_chunk.parent_section == next_chunk.parent_section
            ):
                # Merge chunks
