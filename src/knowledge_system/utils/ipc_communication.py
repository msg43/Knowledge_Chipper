...
class IPCCommunicator:
                ...
                message_data.update(
                    {
                        ...
                        "pid": sys.platform != "win32"
                        and hasattr(sys, "getpid")
                        and sys.getpid()
                        or 0,
                    }
                )
    ...
    def send_error(self, error_message: str, error_code: str | None = None):
        """Send error message."""
        self._send_message(
            {"type": "ERROR", "message": error_message, "error_code": error_code}
        )

    def send_message(self, level: str, message: str):
        """Send general message (info, warning, etc.)."""
        self._send_message({"type": level.upper(), "message": message})

    def send_finished(self, results: dict[str, Any]):
        """Send final completion message with results."""
        self._send_message({"type": "FINISHED", "results": results})

    def send_heartbeat(self, status: str = "alive"):
        """Send heartbeat message to indicate process is alive."""
        self._send_message({"type": "HEARTBEAT", "status": status})
...
class IPCMessageParser:
    ...
    def parse_message(self, message_line: str) -> dict[str, Any] | None:
...
class HeartbeatMonitor:
    ...
    def start_monitoring(self, timeout_callback=None):
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, args=(timeout_callback,), daemon=True
        )
...
class ProcessCommunicationManager:
    ...
    def _handle_message(self, message_data):
        ...
        if callback:
            callback(message_data.get("type", "INFO"), message_data.get("message", ""))
...
def chunk_large_data(
    data: dict[str, Any], max_size: int = 64 * 1024
) -> list[dict[str, Any]]:
