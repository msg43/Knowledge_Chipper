...
class HangDetectionConfig:
    ...
    timeouts: Dict[OperationType, int] = field(default_factory=lambda: {
    ...
    level_multipliers: Dict[HangDetectionLevel, float] = field(default_factory=lambda: {
    ...
    def get_timeout(self, operation_type: OperationType) -> Optional[int]:
...
class HangDetector:
    ...
    def __init__(self, config: HangDetectionConfig):
        self.config = config
    ...
    def start_monitoring(self):
        ...
        get_logger().info(f"Started hang detection monitoring (level: {self.config.level.value})")
    ...
    def unregister_operation(self, operation_id: str):
        ...
        with self._lock:
            if operation_id in self.operations:
                ...
                get_logger().debug(f"Unregistered operation from hang detection: {operation_id}")
    
    def update_operation(self, operation_id: str, metadata: Optional[Dict[str, Any]] = None):
    ...
    def _handle_hung_operation(self, operation: TrackedOperation):
        ...
        if operation.cancellation_token:
            try:
                ...
                get_logger().error(f"Failed to cancel hung operation {operation.operation_id}: {e}")
        
        # Try recovery callback
        if operation.recovery_callback:
            try:
                operation.recovery_callback(operation.operation_id)
                get_logger().info(f"Executed recovery callback for: {operation.operation_id}")
            except Exception as e:
                get_logger().error(f"Recovery callback failed for {operation.operation_id}: {e}")
...
class HangDetectionContext:
    ...
    def __enter__(self):
        ...
        if self.custom_timeout:
            self.detector.config.set_custom_timeout(self.operation_type, self.custom_timeout)
