...
class LLMSpeakerValidator:
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            logger.info(f"LLM Speaker Validator initialized with {self.settings.llm.provider}/{self.settings.llm.model}")
    ...
    def validate_speaker_assignments(
        ...
        try:
            ...
            validation_result = self._parse_llm_response(response.content, proposed_assignments)
            
            logger.info(f"LLM speaker validation completed: {len(validation_result.get('validations', {}))} speakers analyzed")
    ...
    def _prepare_analysis_data(
        ...
        for speaker_id, assigned_name in assignments.items():
            ...
            segment_texts = [seg.get('text', '') for seg in first_segments if seg.get('text')]
            ...
            analysis_data["speakers"][speaker_id] = {
                ...
                "first_appearance": first_segments[0].get('start', 0) if first_segments else 0
            }
...
"""
            for i, text in enumerate(sample_texts[:3], 1):
                speaker_section += f"  {i}. \"{text[:150]}{'...' if len(text) > 150 else ''}\"\n"
...
Focus on accuracy and provide specific reasoning based on the speech content."""
    ...
    def _parse_llm_response(self, response: str, original_assignments: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result = {
                ...
                "validation_summary": llm_result.get("validation_summary", "LLM validation completed"),
                "validations": {},
                "recommendations": {}
            }
            ...
            for speaker_id, original_name in original_assignments.items():
                if speaker_id in speaker_validations:
                    ...
                    recommendation = validation.get("recommendation", "UNCERTAIN")
                    if recommendation == "REJECT" and validation.get("alternative_name"):
                        validation_result["recommendations"][speaker_id] = validation["alternative_name"]
                    elif recommendation == "ACCEPT":
                        validation_result["recommendations"][speaker_id] = original_name
                    else:
                        validation_result["recommendations"][speaker_id] = original_name  # Keep original if uncertain
                else:
    ...
    def create_validation_summary_for_user(self, validation_result: Dict[str, Any]) -> str:
        ...
        accepts = sum(1 for v in validations.values() if v.get("recommendation") == "ACCEPT")
        rejects = sum(1 for v in validations.values() if v.get("recommendation") == "REJECT")
        uncertain = sum(1 for v in validations.values() if v.get("recommendation") == "UNCERTAIN")
        
        confidence_emoji = "🟢" if overall_confidence > 0.8 else "🟡" if overall_confidence > 0.6 else "🔴"
        
        summary_text = f"{confidence_emoji} LLM Analysis (confidence: {overall_confidence:.0%})\n"
        summary_text += f"✅ {accepts} confirmed, ❌ {rejects} rejected, ❓ {uncertain} uncertain\n"
