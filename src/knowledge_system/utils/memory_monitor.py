...
class MemoryMonitor:
    ...
    def check_memory_pressure(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if growth_rate_mb_per_sec > self.growth_rate_threshold:
                pressure_reasons.append(f"High memory growth rate: {growth_rate_mb_per_sec:.1f} MB/s")
            
            # Process-specific checks
            if process_memory_mb > (self.max_memory_gb * 1024 * 0.8):  # 80% of system memory
                pressure_reasons.append(f"Process using {process_memory_mb:.0f}MB ({process_memory_mb/1024:.1f}GB)")
            
            if pressure_reasons:
                return True, "; ".join(pressure_reasons)
            else:
                return False, f"Memory usage normal: {memory.percent:.1f}% RAM, {process_memory_mb:.0f}MB process"
    ...
    def get_memory_stats(self) -> Dict[str, float]:
        ...
        try:
            ...
            stats = {
                ...
                'memory_since_start_mb': (process_memory.rss - self.initial_memory) / (1024 * 1024)
            }
    ...
    def _cleanup_temp_files(self, aggressive: bool = True):
        ...
        if removed_count > 0:
            logger.info(f"Cleaned up {removed_count} temp files, freed {removed_size / (1024*1024):.1f}MB")
    ...
    def _cleanup_system_temp(self):
        ...
        try:
            ...
            if removed_count > 0:
                logger.info(f"Cleaned up {removed_count} files from system temp directory")
