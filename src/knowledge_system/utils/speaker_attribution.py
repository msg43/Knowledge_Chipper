...
class SpeakerAttributor:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = config_path or Path(__file__).parent.parent.parent.parent / "config" / "speaker_attribution.yaml"
        self.load_config()
    ...
    def _create_default_config(self):
        ...
        default_config = {
            'content_detection': {
                'keywords': {
                    'peterson_indicators': [
                        'roughly speaking', 'clean up your room', 'archetypal', 
                        'hierarchies', 'dominance hierarchy', 'responsibility',
                        'meaning', 'chaos', 'order', 'lobster', 'bucko'
                    ],
                    'harris_indicators': [
                        'moral landscape', 'consciousness', 'meditation', 
                        'free will', 'neuroscience', 'rationality', 'illusion',
                        'mindfulness', 'secular', 'dharma'
                    ]
                }
            },
# video_mappings removed - now handled by database learning system
            'speaker_profiles': {
                'Jordan Peterson': {
                    ...
                    'characteristics': ['canadian accent', 'deeper voice', 'measured speech']
                },
                'Sam Harris': {
                    'aliases': ['Harris', 'SH', 'Sam'],
                    'characteristics': ['american accent', 'precise diction', 'analytical tone']
                }
            }
        }
    ...
    def attribute_speakers(self, 
                          ...
                          manual_mapping: Optional[Dict[str, str]] = None) -> List[Dict]:
        """
    ...
    def _apply_mapping(self, segments: List[Dict], mapping: Dict[str, str]) -> List[Dict]:
    ...
    def _detect_speakers_by_content(self, transcript_text: str) -> Dict[str, str]:
        ...
        for speaker_type, keywords in self.content_patterns.items():
            ...
            for keyword in keywords:
                matches = len(re.findall(re.escape(keyword), transcript_text, re.IGNORECASE))
                score += matches
    ...
    def format_transcript_with_speakers(self, segments: List[Dict], format_type: str = "vtt") -> str:
...
def test_speaker_attribution():
    ...
    # Test segments
    test_segments = [
        {"start": 0, "end": 5, "speaker": "Speaker_1", "text": "Roughly speaking, we need to clean up our room"},
        {"start": 6, "end": 10, "speaker": "Speaker_2", "text": "The moral landscape requires consciousness"},
        {"start": 11, "end": 15, "speaker": "Speaker_1", "text": "Hierarchies are archetypal structures"},
    ]
    ...
    result1 = attributor.attribute_speakers(test_segments, transcript_text=test_transcript)
