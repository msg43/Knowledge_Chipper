...
class HardwareDetector:
    ...
    def detect_hardware(self) -> HardwareSpecs:
        ...
        if is_apple_silicon:
            chip_type, gpu_cores, neural_engine_cores, memory_bandwidth_gbps, thermal_design = self._detect_apple_silicon()
        ...
        max_concurrent, optimal_batch, recommended_model, recommended_device = self._calculate_performance_characteristics(
            chip_type, cpu_cores, gpu_cores, memory_gb, thermal_design, gpu_type, cuda_specs
        )
        ...
        logger.info(f"Detected: {chip_type.value} with {cpu_cores} CPU cores, {gpu_cores} GPU cores, {memory_gb}GB RAM")
        return self.specs
    ...
    def _detect_apple_silicon(self) -> Tuple[ChipType, int, int, int, str]:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning("Could not run system_profiler, using fallback detection")
            ...
            chip_type, gpu_cores, neural_engine_cores, memory_bandwidth_gbps, thermal_design = self._parse_chip_specs(chip_name, machine_name)
            
            logger.info(f"Detected Apple Silicon: {chip_type.value}")
            return chip_type, gpu_cores, neural_engine_cores, memory_bandwidth_gbps, thermal_design
            
        except Exception as e:
            logger.warning(f"Apple Silicon detection failed: {e}")
            return self._fallback_apple_silicon_detection()
    
    def _parse_chip_specs(self, chip_name: str, machine_name: str) -> Tuple[ChipType, int, int, int, str]:
    ...
    def _detect_gpu_acceleration(self) -> Tuple[ChipType, GPUType, Optional[CUDASpecs]]:
    ...
    def _detect_cuda(self) -> Optional[CUDASpecs]:
        ...
        try:
            ...
            logger.info(f"Detected CUDA: {gpu_count} GPU(s), {total_vram_gb:.1f}GB VRAM, CUDA {cuda_version}")
            return cuda_specs
    ...
    def _calculate_performance_characteristics(
        self, chip_type: ChipType, cpu_cores: int, gpu_cores: int, memory_gb: int, 
        thermal_design: str, gpu_type: GPUType, cuda_specs: Optional[CUDASpecs]
    ...
    def _recommend_device(self, gpu_type: GPUType, cuda_specs: Optional[CUDASpecs]) -> str:
    ...
    def get_performance_profile(self, profile: PerformanceProfile) -> Dict[str, Any]:
        ...
        if profile == PerformanceProfile.BATTERY_SAVER:
            ...
            return {
                ...
                "max_concurrent_files": max(1, specs.max_concurrent_transcriptions // 2),
                "per_process_thread_limit": max(1, specs.cpu_cores // 4),
                "enable_parallel_processing": True,
                "sequential_processing": False,
                "whisper_model": "base",
                "batch_size": specs.optimal_batch_size // 2,
                "device": specs.recommended_device if specs.recommended_device != "cpu" else "auto",
    ...
    def recommend_profile(self, use_case: str = "general") -> PerformanceProfile:
        ...
        if use_case == "large_batch":
            if specs.chip_type in [ChipType.M3_ULTRA, ChipType.M2_ULTRA, ChipType.M1_ULTRA]:
