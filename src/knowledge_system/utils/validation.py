...
def validate_file_input(
    ...
    if isinstance(input_data, (str, Path)):
        ...
        return all(validate_file_input(item, supported_formats, allow_directories)
...
def validate_text_input(input_path: Union[str, Path]) -> bool:
    """
...
def validate_document_input(input_path: Union[str, Path]) -> bool:
    """
...
def can_process_file(
    input_path: Union[str, Path], supported_formats: List[str]) -> bool:
    """
        ...
        if Path(input_str).exists():
            try:
                with open(input_str, "r", encoding="utf-8") as f:
                    content = f.read()
                    if url_validator:
                        return any(url_validator(line.strip())
