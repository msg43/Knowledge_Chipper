...
class ProcessAnalytics:
    ...
    def end_processing_session(
        self, session_id: str, completed_files: int, failed_files: int, success: bool
    ):
        ...
        with self._lock:
            ...
            if self.metrics.total_crashes > 0:
                metrics.recovery_success_rate = (
                    self.metrics.total_recoveries / self.metrics.total_crashes
                )
            ...
            if recent_speeds:
                metrics.average_processing_speed = sum(recent_speeds) / len(
                    recent_speeds
                )
            ...
            if recent_memory and metrics.average_processing_speed > 0:
                ...
                if avg_memory > 0:
                    metrics.memory_efficiency = (
                        metrics.average_processing_speed / avg_memory
                    )

            # Calculate uptime percentage
            total_events = len(
                [e for e in self.event_history if e["timestamp"] > recent_time]
            )
            error_events = len(
                [
                    e
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        if metrics.average_processing_speed < 1.0:  # Less than 1 file per hour
            health_score -= 10
            issues.append(
                f"Low processing speed: {metrics.average_processing_speed:.2f} files/hour"
            )
        ...
        if metrics.user_satisfaction_score < 7:
            health_score -= 10
            issues.append(
                f"Low user satisfaction: {metrics.user_satisfaction_score:.1f}/10"
            )
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                result = conn.execute(
                    "DELETE FROM performance_samples WHERE timestamp < ?",
                    (cutoff_time,),
                )
                ...
                result = conn.execute(
                    "DELETE FROM crash_reports WHERE timestamp < ?", (cutoff_time,)
                )
                ...
                logger.info(
                    f"Cleaned up old data: {events_deleted} events, {samples_deleted} samples, {crashes_deleted} crashes"
                )
...
def get_process_analytics(db_path: str | None = None) -> ProcessAnalytics:
