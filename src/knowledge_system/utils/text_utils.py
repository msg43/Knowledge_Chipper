...
def estimate_tokens_improved(text: str, model: str = "default") -> int:
    ...
    elif any(local_name in model.lower() for local_name in ["llama", "mistral", "qwen", "codellama"]):
...
def get_model_context_window(model: str) -> int:
    ...
    if model_normalized in MODEL_CONTEXT_WINDOWS:
        return MODEL_CONTEXT_WINDOWS[model_normalized]
    
    # Check for partial matches (for model variants)
...
def calculate_chunking_config(
    ...
    Args:
        ...
        style: Summary style (affects prompt size)
...
3. Uses a {style} style as requested
...
def reassemble_chunk_summaries(
    ...
    Args:
        ...
        original_style: Original summary style requested
        original_prompt_template: Original prompt template
        model: Model name
...
Original style requested: {original_style}
...
4. Uses the {original_style} style as originally requested
5. Provides a comprehensive overview of the entire document
...
def load_interjections(interjections_file: Path) -> Set[str]:
    ...
    try:
        if interjections_file.exists():
            with open(interjections_file, 'r', encoding='utf-8') as f:
                for line in f:
                    line = line.strip()
                    if line and not line.startswith('#'):  # Skip empty lines and comments
                        interjections.add(line)
            logger.info(f"Loaded {len(interjections)} interjections from {interjections_file}")
        else:
