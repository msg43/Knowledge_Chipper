...
class SpeakerColorManager:
    ...
    def get_color_for_speaker(self, speaker_id: str) -> str:
        ...
        if speaker_id not in self.speaker_colors:
            self.speaker_colors[speaker_id] = self.colors[self.color_index % len(self.colors)]
            self.color_index += 1
...
def generate_color_coded_html_transcript(
    ...
    try:
        ...
        return f"<html><body><h1>Error generating transcript</h1><p>{e}</p></body></html>"
    ...
    try:
        ...
        if len(speakers) > 1:
            ...
            for speaker in sorted(speakers):
                color = color_manager.get_color_for_speaker(speaker)
                if use_html_colors:
                    lines.append(f"- <span style='color: {color}; font-weight: bold;'>{speaker}</span>")
                else:
        ...
        for segment in segments:
            ...
            if speaker:
                color = color_manager.get_color_for_speaker(speaker)
                if use_html_colors:
                    segment_lines.append(f"<span style='color: {color}; font-weight: bold; font-size: 1.1em;'>{speaker}</span>")
                else:
            ...
            if include_timestamps:
                ...
                if use_html_colors and speaker:
                    color = color_manager.get_color_for_speaker(speaker)
                    segment_lines.append(f"<span style='color: {color}; opacity: 0.7;'>{timestamp_text}</span>")
                else:
    ...
    try:
        ...
        if output_format.lower() == "html":
            ...
            return generate_color_coded_markdown_transcript(
                updated_data, source_name, model, device, include_timestamps, use_html_colors
            )
        ...
        if speaker_assignments:
            ...
            if output_format == "html":
                ...
                content = generate_color_coded_markdown_transcript(
                    transcript_data, source_name, model, device, include_timestamps, use_html_colors
                )
...
def extract_speaker_statistics(transcript_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """
