...
class GetReceiptsExporter:
    ...
    def __init__(self, config: Optional[GetReceiptsConfig] = None):
    ...
    def export_hce_pipeline_outputs(
        ...
        """
        logger.info(f"Starting GetReceipts export for episode: {pipeline_outputs.episode_id}")
        ...
        try:
            ...
            logger.info(f"Exporting {len(filtered_claims)} of {len(pipeline_outputs.claims)} claims")
            
            # Export each claim as a separate receipt
            results = []
            for i, claim in enumerate(filtered_claims):
                try:
                    logger.info(f"Processing claim {i+1}/{len(filtered_claims)}: {claim.claim_id}")
                    ...
                    if result.get("success"):
                        ...
                        logger.error(f"❌ Failed to export claim {claim.claim_id}: {result.get('error')}")
            ...
            logger.info(f"GetReceipts export complete: {successful_exports} successful, {failed_exports} failed")
            ...
            return {
                ...
                "error": str(e),
                "episode_id": pipeline_outputs.episode_id,
                "claims_processed": len(pipeline_outputs.claims) if pipeline_outputs.claims else 0,
                "claims_exported": 0
            }
    ...
    def _filter_claims(self, claims: List["ScoredClaim"]) -> List["ScoredClaim"]:
        ...
        for claim in claims:
            ...
            confidence = claim.scores.get("confidence_final", claim.scores.get("importance", 0.0))
            if confidence < self.config.min_confidence:
                logger.debug(f"Skipping claim {claim.claim_id}: confidence {confidence} < {self.config.min_confidence}")
                continue
            ...
            if len(filtered) >= self.config.max_claims_per_export:
                logger.info(f"Reached max claims limit: {self.config.max_claims_per_export}")
                break
    ...
    def _build_rf1_receipt(
            ...
            if source_info and source_info.get("url"):
                ...
                if "youtube.com" in video_url or "youtu.be" in video_url:
                    ...
                    if timestamp_seconds is not None:
                        evidence_item["youtube_link"] = f"{video_url}&t={timestamp_seconds}s"
            
            evidence_list.append(evidence_item)
        
        # Find related claims
        related_claims = self._find_related_claims(claim, pipeline_outputs.relations, pipeline_outputs.claims)
        ...
        knowledge_artifacts = {
            ...
            "mental_models": self._format_mental_model_artifacts(pipeline_outputs.concepts),
        ...
        receipt = {
            ...
            "topics": [claim.claim_type] + self._extract_topics_from_context(episode_context),
        ...
        logger.debug(f"Built RF-1 receipt for claim {claim.claim_id} with {len(evidence_list)} evidence items")
        return receipt
    ...
    def _build_detailed_claim_description(
        ...
        if evidence_list:
            ...
            for i, evidence in enumerate(evidence_list[:3], 1):  # Limit to top 3
                description_parts.append(f"{i}. {evidence['text']} (at {evidence['timestamp']})")
        ...
        if scores:
            ...
            if "confidence_final" in scores:
                description_parts.append(f"• Confidence: {scores['confidence_final']:.2f}")
            if "controversy" in scores:
                description_parts.append(f"• Controversy: {scores['controversy']:.2f}")
        
        return "\n".join(description_parts)
    
    def _extract_topics_from_context(self, episode_context: Optional[str]) -> List[str]:
    ...
    def _format_sources(
        ...
        if source_info:
            ...
            sources.append({
                ...
                "evidence_timestamps": [e.get("youtube_link") for e in evidence_list if e.get("youtube_link")]
            })
        
        return sources
    
    def _format_people_artifacts(self, people: List["PersonMention"]) -> List[Dict[str, Any]]:
    ...
    def _format_jargon_artifacts(self, jargon: List["JargonTerm"]) -> List[Dict[str, Any]]:
        ...
        for term in jargon:
            ...
            formatted.append({
                ...
                "usage_examples": [e.quote for e in term.evidence_spans[:3]]  # Top 3 examples
            })
        
        return formatted
    
    def _format_mental_model_artifacts(self, concepts: List["MentalModel"]) -> List[Dict[str, Any]]:
        ...
        for model in concepts:
            ...
            formatted.append({
                ...
                "evidence_timestamps": evidence_timestamps,
                "key_concepts": [e.quote for e in model.evidence_spans[:3]],  # Top 3 concepts
    ...
    def _submit_receipt_with_retry(self, receipt: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                logger.debug(f"Submitting receipt (attempt {attempt + 1}/{self.config.max_retries})")
                ...
                if response.status_code == 200 or response.status_code == 201:
                    ...
                    logger.warning(f"Receipt submission failed: HTTP {response.status_code}")
...
def create_exporter_from_settings(settings: Optional[Dict[str, Any]] = None) -> GetReceiptsExporter:
    ...
    config = GetReceiptsConfig(
        base_url=getreceipts_config.get("base_url", "https://getreceipts-web.vercel.app"),
        ...
        include_evidence_timestamps=getreceipts_config.get("include_evidence_timestamps", True)
    )
