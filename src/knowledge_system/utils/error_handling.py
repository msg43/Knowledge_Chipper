...
class ErrorHandler:
    ...
    def create_error_result(
        ...
        metadata = {
            ...
            "error_type": type(error).__name__ if isinstance(error, Exception) else "UserError",
            "timestamp": time.time(),
        }
...
class YouTubeErrorHandler:
    ...
    ERROR_PATTERNS = {
        "proxy_auth": {
            "patterns": ["407 Proxy Authentication Required", "Proxy authentication failed"],
            "message": "🔐 Proxy authentication failed: Please check your WebShare Username and Password in Settings",
            "category": "authentication"
        },
        "proxy_connection": {
            "patterns": ["Proxy connection failed", "Tunnel connection failed", "ProxyError"],
        ...
        "video_unavailable": {
            "patterns": ["Video unavailable", "This video is not available", "Private video", "Deleted video"],
    ...
    def create_youtube_error_result(
        ...
        metadata = {
            "processor": processor_name,
            "error_type": type(error).__name__ if isinstance(error, Exception) else "YouTubeError",
            "url": url,
            "raw_error": error_msg,
            "timestamp": time.time(),
        }
            ...
            try:
                ...
                if error_handler:
                    ...
                    return ErrorHandler.create_error_result(
                        error=e,
                        processor_name=proc_name,
                        context={"input_args": len(args), "input_kwargs": list(kwargs.keys())},
                        dry_run=kwargs.get('dry_run', False)
                    )
...
def with_youtube_error_handling(processor_name: Optional[str] = None):
    ...
    """
    def youtube_error_handler(error, proc_name, *args, **kwargs):
...
class BatchErrorHandler:
    ...
    def create_batch_summary_result(
        ...
        return ProcessorResult(
            ...
            errors=error_analysis["all_errors"] if error_analysis["all_errors"] else None,
            metadata={
                "operation": operation_name,
...
def file_not_found_error(file_path: Union[str, Path], processor_name: str = "processor") -> ProcessorResult:
...
def network_error(url: str, error_msg: str, processor_name: str = "processor") -> ProcessorResult:
