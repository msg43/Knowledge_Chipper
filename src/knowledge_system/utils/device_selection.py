...
from .hardware_detection import get_hardware_detector, GPUType, CUDASpecs
...
def select_optimal_device(
    ...
    if preference != "auto":
        if _validate_device_capability(preference, model_size, batch_size):
            ...
            logger.warning(f"User preference '{preference}' not optimal for workload, falling back to auto selection")
    
    # Auto selection logic
    detector = get_hardware_detector()
    specs = detector.detect_hardware()
    
    logger.info(f"Auto-selecting device for {workload_type} workload with {model_size} model")
    
    # CUDA gets highest priority for suitable workloads
    if specs.supports_cuda and specs.cuda_specs:
        cuda_device = _evaluate_cuda_suitability(specs.cuda_specs, model_size, batch_size, workload_type)
        if cuda_device:
            logger.info(f"Selected CUDA: {specs.cuda_specs.gpu_names[0]} ({specs.cuda_specs.total_vram_gb:.1f}GB VRAM)")
            return cuda_device
    ...
    if specs.supports_mps:
        if _evaluate_mps_suitability(specs, model_size, batch_size, workload_type):
            logger.info(f"Selected MPS: {specs.chip_type.value} GPU ({specs.gpu_cores} cores)")
            return "mps"
...
def _validate_device_capability(device: str, model_size: str, batch_size: int) -> bool:
    ...
    if device == "cuda":
        if not specs.supports_cuda or not specs.cuda_specs:
            ...
            return False
        return _evaluate_cuda_suitability(specs.cuda_specs, model_size, batch_size, "validation") is not None
...
def _evaluate_cuda_suitability(cuda_specs: CUDASpecs, model_size: str, batch_size: int, workload_type: str) -> Optional[str]:
    ...
    if cuda_specs.total_vram_gb < estimated_vram_needed:
        logger.debug(f"CUDA GPUs have {cuda_specs.total_vram_gb:.1f}GB VRAM, need ~{estimated_vram_needed:.1f}GB for {model_size} model")
        return None
    ...
    # Not enough VRAM
    logger.debug(f"CUDA GPUs insufficient for workload (need {estimated_vram_needed:.1f}GB, have {cuda_specs.total_vram_gb:.1f}GB)")
    return None


def _evaluate_mps_suitability(specs, model_size: str, batch_size: int, workload_type: str) -> bool:
    ...
    if specs.memory_gb < estimated_memory_needed:
        logger.debug(f"MPS needs {estimated_memory_needed}GB RAM, have {specs.memory_gb}GB")
        return False
...
def get_device_recommendations(workload_type: str = "transcription") -> Dict[str, Any]:
    ...
    if specs.supports_cuda and specs.cuda_specs:
        ...
        if specs.cuda_specs.supports_tensor_cores:
            recommendations["device_info"]["cuda"]["features"].append("Tensor Cores (mixed precision)")
        
        if specs.cuda_specs.total_vram_gb >= 16:
            recommendations["performance_notes"].append("High VRAM enables large-v3 model with large batch sizes")
        elif specs.cuda_specs.total_vram_gb >= 8:
            recommendations["performance_notes"].append("Good VRAM for medium to large models")
    ...
    if specs.supports_mps:
        ...
        recommendations["device_info"]["mps"] = {
            ...
            "suitable_for": ["optimized for Apple Silicon", "energy efficient", "good performance"],
            "performance": "high" if specs.gpu_cores >= 16 else "medium"
        }
        
        if specs.memory_gb >= 32:
            recommendations["performance_notes"].append("High unified memory enables large models")
