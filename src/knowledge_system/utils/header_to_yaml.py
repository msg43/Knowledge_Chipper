        ...
        if config_file.exists():
            with open(config_file, 'r', encoding='utf-8') as f:
                ...
                if content:
                    ...
                    headers = [header.strip() for header in content.split(';') if header.strip()]
    ...
    try:
        ...
        for pattern in header_patterns:
            ...
            for match in matches:
                ...
                logger.debug(f"Section content for '{header}': {repr(section_content[:200])}...")
                ...
                for bullet_pattern in bullet_patterns:
                    bullet_matches = re.findall(bullet_pattern, section_content, re.MULTILINE)
                    for bullet_text in bullet_matches:
                        ...
                        if cleaned_text and cleaned_text not in bullet_points:
                            ...
                            if header.lower() in ["mental models", "jargon"]:
                                ...
                                clean_term = re.split(r'\s*[-–—:]\s*', cleaned_text)[0].strip()
                                if clean_term:
                                    bullet_points.append(clean_term)
                                    logger.debug(f"Extracted clean term for {header}: '{clean_term}' from '{cleaned_text}'")
                            else:
        ...
        logger.debug(f"Found {len(bullet_points)} bullet points under header '{header}'")
...
def sanitize_yaml_field_name(header: str) -> str:
    ...
    clean_header = re.sub(r'[^a-zA-Z0-9\s-]', ' ', clean_header)  # Replace special chars with spaces
...
def generate_yaml_fields(header: str, bullet_points: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        logger.debug(f"Generated {len(yaml_fields)} YAML fields for header '{header}' -> '{clean_header}'")
...
def process_summary_for_yaml_headers(content: str, analysis_type: str) -> Dict[str, str]:
    """
    ...
    if analysis_type.lower() != "document summary":
        logger.debug(f"Skipping header-to-YAML processing for analysis type: {analysis_type}")
    ...
    try:
        ...
        if not header_phrases:
            logger.warning("No header phrases loaded, skipping header-to-YAML processing")
            return additional_yaml_fields
        ...
        for header in header_phrases:
            ...
            if bullet_points:
                ...
                logger.info(f"Added {len(yaml_fields)} YAML fields for header '{header}'")
            else:
                logger.debug(f"No bullet points found for header '{header}'")
        
        logger.info(f"Generated {len(additional_yaml_fields)} additional YAML fields from headers")
