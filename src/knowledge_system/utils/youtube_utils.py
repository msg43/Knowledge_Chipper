...
def get_manual_cookie_file() -> Optional[Path]:
    """
...
def _show_cookie_help_dialog(error_message: Optional[str] = None):
    """
    ...
    try:
        ...
        if error_message and "sign in to confirm you're not a bot" in error_message.lower():
...
def get_single_working_strategy() -> Dict[str, Any]:
    ...
    try:
        ...
        for browser_name, profile in browsers_to_try:
            ...
            try:
                ...
                if cookie_jar and len(list(cookie_jar)) > 0:
                    logger.info(f"Loaded {len(list(cookie_jar))} cookies from {browser_name}.")
        ...
        logger.warning("yt_dlp cookie importer not available. Cannot load browser cookies.")
    except Exception as e:
    ...
    try:
        ...
        for quality, thumbnail_image_url in thumbnail_configs:
            try:
                ...
                response = requests.get(thumbnail_image_url, headers=headers, timeout=10)
                if response.status_code == 200:
                    ...
                    if content_length > 1000:  # Valid thumbnails are typically larger than 1KB
                        ...
                        logger.debug(f"{quality} thumbnail too small ({content_length} bytes), trying next quality")
                        continue
                else:
                    logger.debug(f"{quality} thumbnail not available (status {response.status_code})")
                    continue
...
def download_thumbnail(url: str, output_dir: Path, use_cookies: bool = False, thumbnail_url: Optional[str] = None) -> Optional[str]:
    """
...
def download_thumbnails_batch(
    urls: List[str], output_dir: Path, timeout: int = 30, use_cookies: bool = False) -> List[Optional[str]]:
    """
...
def get_authentication_status() -> Dict[str, Any]:
    ...
    status = {
        ...
        "cache_age_seconds": (now - _cookie_jar_timestamp) if _cookie_jar_cache else None,
        "manual_cookie_file_found": str(get_manual_cookie_file()) if get_manual_cookie_file() else "No",
    }
    ...
    try:
        ...
        if platform.system() == "Darwin":  # macOS
            try:
                ...
                logger.debug(f"AppleScript network optimization failed: {e}")  # Fallback if AppleScript fails
