...
class ModelPreloader(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self._diarization_thread: threading.Thread | None = None
    ...
    def configure(
        ...
        logger.info(
            f"Model preloader configured: model={model}, device={device}, diarization={enable_diarization}"
        )
    ...
    def start_preloading(self):
        ...
        if os.environ.get("KNOWLEDGE_CHIPPER_TESTING_MODE") == "1":
            logger.info(
                "🧪 Testing mode: Skipping all model preloading to avoid threading issues"
            )
            return
    ...
    def _load_transcription_model(self):
        ...
        try:
            ...
            if (
                ...
                and not self.diarization_loading
                and not self.diarization_ready
            ):
