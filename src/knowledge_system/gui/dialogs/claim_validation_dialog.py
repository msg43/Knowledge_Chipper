...
class ClaimCard(QFrame):
    ...
    def setup_ui(self):
        ...
        for tier in ["A", "B", "C"]:
            ...
            btn.toggled.connect(
                lambda checked, t=tier: self._on_tier_changed(t, checked)
            )
    ...
    def _confirm_claim(self):
        ...
        self.claim_confirmed.emit(
            self.claim_data.get("claim_id", ""), self.current_tier, self.was_modified
        )
    ...
    def get_validation_result(self) -> dict[str, Any] | None:
