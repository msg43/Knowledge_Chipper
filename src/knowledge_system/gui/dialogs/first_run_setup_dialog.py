...
class FirstRunSetupDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        if os.environ.get("KNOWLEDGE_CHIPPER_TESTING_MODE"):
            ...
            logger = get_logger(__name__)
            logger.error(
                "🧪 CRITICAL: Attempted to create FirstRunSetupDialog during testing mode - BLOCKED!"
            )
            raise RuntimeError(
                "FirstRunSetupDialog cannot be created during testing mode"
            )
    ...
    def _create_welcome_message(self, layout: QVBoxLayout):
        ...
        message.setStyleSheet(
            "color: #333; padding: 10px; background: #f8f9fa; border-radius: 8px;"
        )
    ...
    def _create_buttons(self, layout: QVBoxLayout):
        ...
        self.buttons.addButton(
            self.download_button, QDialogButtonBox.ButtonRole.AcceptRole
        )
    ...
    def _update_selected_models(self):
        ...
        if self.selected_models:
            ...
            self.download_button.setText(
                f"Download {len(self.selected_models)} Models ({total_size})"
            )
    ...
    def _start_download(self):
        ...
        self.download_worker.all_downloads_completed.connect(
            self._on_all_downloads_completed
        )
    ...
    def _update_progress(self, progress_info: dict):
        ...
        # Update progress bar
        overall_progress = ((current_model - 1) / total_models) * 100 + (
            percent / total_models
        )
        self.progress_bar.setValue(int(overall_progress))

        # Update labels
        self.progress_label.setText(
            f"Downloading {model_name} model... ({current_model}/{total_models})"
        )
    ...
    def _on_all_downloads_completed(self, success: bool):
        ...
        if success:
            ...
            self.download_status.setText(
                "You can retry downloading models later from settings"
            )
