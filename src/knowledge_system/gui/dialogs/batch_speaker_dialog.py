...
class RecordingItem:
    ...
    def __init__(self, file_path: Path, speaker_data_list: List[SpeakerData]):
        ...
        if speaker_data_list:
            total_confidence = sum(speaker.confidence_score for speaker in speaker_data_list)
        ...
        try:
            ...
            for i, recording in enumerate(self.recordings):
                ...
                self.progress_updated.emit(
                    progress, 
                    f"Processing {recording.file_path.name}..."
                )
...
class BatchSpeakerAssignmentDialog(QDialog):
    ...
    def _create_header(self) -> QHBoxLayout:
        ...
        total_speakers = sum(recording.get_speaker_count() for recording in self.recordings)
        total_duration = sum(recording.get_total_duration() for recording in self.recordings)
        ...
        stats_label.setStyleSheet("color: #888; background-color: #f5f5f5; padding: 8px; border-radius: 4px;")
    ...
    def _create_recording_list_panel(self) -> QWidget:
        ...
        for i, recording in enumerate(self.recordings):
            ...
            confidence_text = f"{recording.overall_confidence:.0%}" if recording.overall_confidence > 0 else "N/A"
    ...
    def _create_current_recording_panel(self) -> QWidget:
        ...
        self.assign_speakers_btn.setStyleSheet("background-color: #4caf50; font-weight: bold;")
    ...
    def _create_options_section(self) -> QGroupBox:
        ...
        # Consistency options
        self.maintain_consistency_cb = QCheckBox("Maintain speaker consistency across recordings")
        self.maintain_consistency_cb.setChecked(True)
        self.maintain_consistency_cb.setToolTip("Try to assign the same names to similar speakers across different recordings")
        ...
        self.learn_voices_cb.setToolTip("Save speaker voice characteristics for automatic suggestions in future recordings")
    ...
    def _create_button_box(self) -> QDialogButtonBox:
        ...
        button_box.button(QDialogButtonBox.StandardButton.Ok).setText("Process All Recordings")
    ...
    def _analyze_batch_consistency(self):
        ...
        try:
            ...
            folder_path = self.recordings[0].file_path.parent if self.recordings else None
            
            if folder_path:
                # Get suggestions for consistent speakers in this folder
                consistency_analysis = self.name_suggester.analyze_speaker_consistency(folder_path)
                consistent_speakers = consistency_analysis.get("consistent_speakers", [])
                ...
                logger.info(f"Found {len(consistent_speakers)} consistent speakers for batch processing")
    ...
    def _update_speaker_area(self, recording: RecordingItem):
        ...
        if not recording.speaker_data_list:
            ...
            for speaker_data in recording.speaker_data_list:
                speaker_frame = self._create_speaker_preview_frame(speaker_data, recording)
                layout.addWidget(speaker_frame)
        
        layout.addStretch()
        self.speaker_area.setWidget(widget)
    
    def _create_speaker_preview_frame(self, speaker_data: SpeakerData, recording: RecordingItem) -> QFrame:
        """Create a preview frame for a speaker."""
        frame = QFrame()
        frame.setFrameStyle(QFrame.Shape.Box)
        frame.setStyleSheet("QFrame { border: 1px solid #ddd; border-radius: 4px; padding: 8px; margin: 4px; }")
        ...
        if speaker_data.sample_texts:
            sample_text = speaker_data.sample_texts[0][:100] + "..." if len(speaker_data.sample_texts[0]) > 100 else speaker_data.sample_texts[0]
    ...
    def _show_speaker_assignment_dialog(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            logger.info(f"Updated assignments for {recording.file_path.name}: {assignments}")
    ...
    def _apply_consistent_assignments(self):
        ...
        for i, speaker_data in enumerate(recording.speaker_data_list):
            ...
            if speaker_id in self.consistent_speakers:
                recording.assignments[speaker_data.speaker_id] = self.consistent_speakers[speaker_id]
