...
class CrashRecoveryDialog(QDialog):
        ...
        try:
                ...
                if location.exists():
                    ...
                    for checkpoint_file in location.glob("*checkpoint*.json"):
                        try:
                            ...
                            logger.warning(
                                f"Failed to analyze checkpoint {checkpoint_file}: {e}"
                            )
            ...
            for checkpoint_file in temp_dir.glob("kc_checkpoint_*.json"):
                try:
                    ...
                    logger.warning(
                        f"Failed to analyze temp checkpoint {checkpoint_file}: {e}"
                    )
    ...
    def _analyze_checkpoint(self, checkpoint_file: Path) -> dict | None:
        ...
        try:
            ...
            if job_name.startswith("kc_checkpoint_"):
                job_name = (
                    f"Batch Job {checkpoint_file.stem[-8:]}"  # Last 8 chars of filename
                )
    ...
    def _determine_job_status(
        self, checkpoint_data: dict, checkpoint_file: Path
    ) -> str:
    ...
    def _update_table(self):
        ...
        if self.checkpoint_jobs:
            self.setWindowTitle(
                f"Processing Recovery ({len(self.checkpoint_jobs)} jobs found)"
            )
        else:
    ...
    def _update_details(self):
        ...
        details.append(
            f"Progress: {job['completed_files']}/{job['total_files']} files ({job['progress_percent']:.1f}%)"
        )
        details.append(
            f"Last Updated: {job['last_updated'].strftime('%Y-%m-%d %H:%M:%S')}"
        )
        details.append("")
        ...
        if config:
            ...
            details.append(
                f"  • Transcription: {'Yes' if config.get('transcribe') else 'No'}"
            )
            details.append(
                f"  • Summarization: {'Yes' if config.get('summarize') else 'No'}"
            )
            details.append(
                f"  • MOC Generation: {'Yes' if config.get('create_moc') else 'No'}"
            )
            if config.get("transcription_model"):
                details.append(
                    f"  • Transcription Model: {config['transcription_model']}"
                )
            details.append("")
        ...
        try:
            ...
            def _do_show():
                if self.dialog is None:
                    ...
                    self.dialog.recovery_action_selected.connect(
