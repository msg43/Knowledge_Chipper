    ...
    def run(self):
        ...
        try:
            if self.table:
                ...
                result = self.sync_service.sync_table(
                    self.table, self.conflict_resolution
                )
            else:
...
class SyncStatusTab(QWidget):
    ...
    def create_details_panel(self, parent_layout):
        ...
        self.resolution_combo.addItems(["Manual", "Local Wins", "Remote Wins", "Merge"])
    ...
    def sync_all(self):
        ...
        if self.sync_worker and self.sync_worker.isRunning():
            QMessageBox.warning(
                self, "Sync in Progress", "A sync operation is already running"
            )
            return
        ...
        resolution = resolution_map.get(
            self.resolution_combo.currentText(), ConflictResolution.MANUAL
        )
    ...
    def on_sync_finished(self, result: SyncResult):
        ...
        if result.success:
            ...
            QMessageBox.warning(
                self, "Sync Failed", "Sync operation failed. Check the log for details."
            )
    ...
    def update_conflicts_table(self):
        ...
        for i, conflict in enumerate(self.conflicts):
            ...
            self.conflicts_table.setItem(
                i,
                3,
                QTableWidgetItem(conflict.local_updated.strftime("%Y-%m-%d %H:%M")),
            )
            self.conflicts_table.setItem(
                i,
                4,
                QTableWidgetItem(conflict.remote_updated.strftime("%Y-%m-%d %H:%M")),
            )
    ...
    def view_conflict_diff(self):
        ...
        diff_dialog.setWindowTitle(
            f"Conflict: {conflict.table_name}:{conflict.record_id}"
        )
