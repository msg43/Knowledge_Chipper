...
from PyQt6.QtWidgets import (
    ...
    QCheckBox, QListWidget, QFileDialog, QMessageBox, QProgressBar, QWidget
)
...
class EnhancedTranscriptionWorker(QThread):
    ...
    transcription_step_updated = pyqtSignal(str, int)  # step_description, progress_percent
    ...
    def _transcription_progress_callback(self, step_description_or_dict, progress_percent: int = 0):
        ...
        if isinstance(step_description_or_dict, dict):
            ...
            if status == "starting_download":
                message = progress_dict.get("message", f"Starting download of {model} model...")
                ...
                message = progress_dict.get("message", f"Successfully downloaded {model} model")
    ...
    def run(self):
        ...
        try:
            ...
            valid_audio_processor_params = {
                'normalize_audio', 'target_format', 'device', 'temp_dir', 
                'use_whisper_cpp', 'model', 'progress_callback', 'enable_diarization', 'hf_token',
                'enable_quality_retry', 'max_retry_attempts'
            }
            ...
            processor = AudioProcessor(
                ...
                enable_quality_retry=self.gui_settings.get("enable_quality_retry", True),
            ...
            for i, file_path in enumerate(self.files):
                ...
                self.transcription_step_updated.emit(f"Starting transcription of {file_name}...", 0)
                ...
                try:
                    ...
                    if result.success:
                        ...
                        text_length = len(transcript_data.get('text', '')) if transcript_data else 0
                        ...
                        self.progress_updated.emit(progress_data)
                        self.transcription_step_updated.emit(f"❌ Transcription of {file_name} failed", 0)
                    ...
                    self.progress_updated.emit(progress_data)
                    self.transcription_step_updated.emit(f"❌ Error transcribing {file_name}: {str(e)}", 0)
...
class TranscriptionTab(BaseTab, FileOperationsMixin):
    ...
    def _create_settings_section(self) -> QGroupBox:
        ...
        self.overwrite_checkbox.setToolTip("If unchecked, existing transcripts will be skipped")
        ...
        quality_info_label = QLabel("💡 Tip: Disable retry for fastest processing, enable for best quality")
        quality_info_label.setStyleSheet("color: #666; font-size: 10px; font-style: italic;")
    ...
    def _start_processing(self):
        ...
        self.transcription_worker = EnhancedTranscriptionWorker(files, self.settings, gui_settings, self)
        ...
        self.transcription_worker.transcription_step_updated.connect(self._update_transcription_step)
    ...
    def _update_transcription_step(self, step_description: str, progress_percent: int):
        ...
        if hasattr(self, 'file_progress_bar') and hasattr(self, 'progress_status_label'):
    ...
    def _update_progress(self, progress_data):
        ...
        if isinstance(progress_data, dict):
            ...
            if success and text_length > 0:
                self.append_log(f"[{current}/{total}] {file_name}: {status_icon} {status}")
                self.append_log(f"   📝 Generated {text_length:,} characters of transcription text")
            else:
                self.append_log(f"[{current}/{total}] {file_name}: {status_icon} {status}")
        else:
    ...
    def _processing_finished(self):
        ...
        self.append_log("📋 Note: Transcriptions are processed in memory. Use the Process Pipeline tab to save transcripts to markdown files.")
    ...
    def _apply_recommended_settings(self):
        ...
        try:
            ...
            # Model
            if specs.recommended_whisper_model in ["tiny", "base", "small", "medium", "large"]:
            ...
            recommended_device = device_mapping.get(specs.recommended_device.lower(), "auto")
            ...
            # Max concurrent files
            if hasattr(specs, 'max_concurrent_transcriptions') and specs.max_concurrent_transcriptions:
    ...
    def _get_transcription_settings(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hf_token': getattr(self.settings.api_keys, 'huggingface_token', None),  # Add HF token for diarization
            'kwargs': {
                'diarization': self.diarization_checkbox.isChecked(),
                'omp_threads': self.omp_threads.value(),
                'batch_size': self.batch_size.value(),
                'hf_token': getattr(self.settings.api_keys, 'huggingface_token', None)  # Also add to kwargs for AudioProcessor
            }
        }
        ...
        if self.diarization_checkbox.isChecked():
            ...
            if not hf_token:
                self.show_warning(
                    ...
                    "Please configure your HuggingFace token in the API Keys tab or disable speaker diarization.\n\n"
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.quality_retry_checkbox.setChecked(
                self.gui_settings.get_checkbox_state(self.tab_name, "enable_quality_retry", True)
            )
            ...
            self.max_retry_attempts.setValue(
                self.gui_settings.get_spinbox_value(self.tab_name, "max_retry_attempts", 1)
            )
            
            # Load processing mode selection
            saved_mode = self.gui_settings.get_combo_selection(self.tab_name, "processing_mode", "balanced")
    ...
    def _save_settings(self):
        ...
        try:
            ...
            self.gui_settings.set_combo_selection(self.tab_name, "device", self.device_combo.currentText())
            self.gui_settings.set_combo_selection(self.tab_name, "language", self.language_combo.currentText())
            self.gui_settings.set_combo_selection(self.tab_name, "format", self.format_combo.currentText())
            self.gui_settings.set_combo_selection(self.tab_name, "processing_mode", self.processing_mode.currentText())
            
            # Save checkbox states
            self.gui_settings.set_checkbox_state(self.tab_name, "include_timestamps", self.timestamps_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "enable_diarization", self.diarization_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "overwrite_existing", self.overwrite_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "enable_quality_retry", self.quality_retry_checkbox.isChecked())
            
            # Save spinbox values
            self.gui_settings.set_spinbox_value(self.tab_name, "batch_size", self.batch_size.value())
            self.gui_settings.set_spinbox_value(self.tab_name, "omp_threads", self.omp_threads.value())
            self.gui_settings.set_spinbox_value(self.tab_name, "max_concurrent", self.max_concurrent.value())
            self.gui_settings.set_spinbox_value(self.tab_name, "max_retry_attempts", self.max_retry_attempts.value())
    ...
    def _on_quality_retry_toggled(self, checked: bool):
        ...
        if checked:
            ...
            self.max_retry_attempts.setToolTip("Disabled because automatic quality retry is turned off") 