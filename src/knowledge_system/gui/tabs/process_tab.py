    ...
    def run(self):
        ...
        try:
            ...
            for i, file_path in enumerate(self.files):
                ...
                self.progress_updated.emit(i + 1, len(self.files), f"Processing {file_path_obj.name}")
                
                try:
                    # Step 1: Transcription (if enabled and file is audio/video)
                    transcript_path = None
                    if (self.config['transcribe'] and audio_processor and 
                        file_path_obj.suffix.lower() in ['.mp4', '.mp3', '.wav', '.m4a', '.avi', '.mov', '.mkv']):
                        ...
                        try:
                            ...
                            if result.success:
                                ...
                                if hasattr(result, 'data') and result.data:
                                    if isinstance(result.data, dict) and 'output_file' in result.data:
                                        transcript_path = Path(result.data['output_file'])
                                        moc_input_files.append(transcript_path)
                                        
                                self.file_completed.emit(
                                    str(file_path_obj), True, 
                                    f"✅ Transcribed: {file_path_obj.name}"
                                )
                            else:
                                error_msg = '; '.join(result.errors) if result.errors else 'Unknown error'
                                results['errors'].append(f"Transcription failed for {file_path_obj}: {error_msg}")
                                self.file_completed.emit(
                                    str(file_path_obj), False,
                                    f"❌ Transcription failed: {error_msg}"
                                )
                        except Exception as e:
                            error_msg = f"Transcription error: {e}"
                            results['errors'].append(f"{file_path_obj}: {error_msg}")
                            self.file_completed.emit(str(file_path_obj), False, f"❌ {error_msg}")
                    
                    # Step 2: Summarization (if enabled and file is document/transcript)
                    if (self.config['summarize'] and summarizer_processor and
                        file_path_obj.suffix.lower() in ['.pdf', '.txt', '.md']):
                        
                        # Use transcript if it was just created, otherwise use original file
                        summarize_file = transcript_path if transcript_path else file_path_obj
                        ...
                        try:
                            ...
                            if result.success:
                                ...
                                error_msg = '; '.join(result.errors) if result.errors else 'Unknown error'
                                results['errors'].append(f"Summarization failed for {file_path_obj}: {error_msg}")
                                self.file_completed.emit(
                                    str(file_path_obj), False,
            ...
            if self.config['moc'] and moc_processor and moc_input_files and not self.should_stop:
                self.progress_updated.emit(len(self.files), len(self.files), "Generating Maps of Content...")
                ...
                try:
                    ...
                    if unique_files:
                        ...
                        if result.success:
                            ...
                            error_msg = '; '.join(result.errors) if result.errors else 'Unknown error'
                            results['errors'].append(f"MOC generation failed: {error_msg}")
                            self.file_completed.emit(
                                "MOC Generation", False,
...
class ProcessTab(BaseTab, FileOperationsMixin):
    ...
    def _create_progress_section(self) -> QGroupBox:
        ...
        self.file_progress_list.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.MinimumExpanding)
    ...
    def _get_inherited_config(self):
        ...
        if transcription_tab:
            ...
            config['device'] = config['device'].currentText() if config['device'] else 'auto'
            config['transcription_model'] = getattr(transcription_tab, 'model_combo', None)
            config['transcription_model'] = config['transcription_model'].currentText() if config['transcription_model'] else 'base'
        else:
            config['device'] = 'auto'
        ...
        if summarization_tab:
            ...
            config['summarization_provider'] = provider.currentText() if provider else 'openai'
            config['summarization_model'] = model.currentText() if model else 'gpt-4o-mini-2024-07-18'
        ...
        if self.processing_worker and self.processing_worker.isRunning():
            ...
            if hasattr(self.processing_worker, 'paused'):
                ...
                if self.processing_worker.paused:
                    ...
                    self.pause_btn.setStyleSheet("background-color: #4caf50; font-weight: bold;")
    ...
    def _start_processing(self):
        ...
        if not (config['transcribe'] or config['summarize'] or config['moc']):
            self.show_warning("No Operations", "Please enable at least one processing operation")
            return
        ...
        if config['summarize']:
            self.append_log(f"  Summarization: {config['summarization_provider']} {config['summarization_model']}")
    ...
    def _processing_finished(self, results: Dict[str, Any]):
        ...
        if results['moc_generated']:
            self.append_log(f"MOC generated from: {len(results['moc_generated'])} files")
        if results['errors']:
    ...
    def _save_settings(self):
        ...
        try:
            ...
            self.gui_settings.set_output_directory(self.tab_name, self.output_directory.text())
            
            # Save checkbox states
            self.gui_settings.set_checkbox_state(self.tab_name, "transcribe", self.transcribe_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "summarize", self.summarize_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "moc", self.moc_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "dry_run", self.dry_run_checkbox.isChecked())
