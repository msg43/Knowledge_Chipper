...
from PyQt6.QtCore import Qt, QThread, QTimer, QProcess, pyqtSignal
...
class ProcessPipelineWorker(QProcess):
    ...
    def _handle_progress(self, current_file: int, total_files: int, message: str, 
                        progress: int | None, stage: str):
    ...
    def _handle_process_finished(self, exit_code: int, exit_status):
        ...
        if exit_code == 0:
            ...
            if self.restart_attempts < self.max_restart_attempts:
                ...
                delay = min(300, 2 ** self.restart_attempts) * 1000  # Exponential backoff in ms
                logger.info(f"Restarting worker process in {delay/1000}s (attempt {self.restart_attempts})")
                QTimer.singleShot(delay, self._restart_processing)
            else:
                self.processing_error.emit(f"Maximum restart attempts exceeded: {error_msg}")
    ...
    def _build_command(self) -> list:
        ...
        if not self.checkpoint_file:
            checkpoint_fd, self.checkpoint_file = tempfile.mkstemp(suffix='.json', prefix='kc_checkpoint_')
            os.close(checkpoint_fd)
        
        # Build command
        cmd = [
            sys.executable, '-m', 'knowledge_system.workers.batch_processor_main',
            '--files'] + [str(f) for f in self.files] + [
            '--config', config_path,
            '--output-dir', str(self.output_dir),
            '--checkpoint-file', str(self.checkpoint_file),
            '--log-level', 'INFO'
        ]
    ...
    def start_processing(self):
        ...
        try:
            ...
            self.comm_manager.start_heartbeat_monitoring(
                timeout_callback=lambda: self.processing_error.emit("Worker process heartbeat timeout")
            )
...
class ProcessTab(BaseTab, FileOperationsMixin):
    ...
    def _add_directory(self) -> None:
        ...
        if directory:
            supported_extensions = ['.mp3', '.wav', '.m4a', '.mp4', '.avi', '.mkv', '.pdf', '.txt', '.md']
    ...
    def _start_processing(self) -> None:
        ...
        try:
            ...
            config = {
                ...
                "summarization_provider": "openai",  # Default for now
                "summarization_model": "gpt-4",  # Default for now
                "moc_provider": "openai",  # Default for now
                "moc_model": "gpt-4"  # Default for now
            }
    ...
    def _processing_finished(self, results: dict) -> None:
        ...
        self.status_label.setText(f"Processing completed: {files_processed}/{total_files} files processed, {files_failed} failed")
        
        # Show completion message
        if files_failed == 0:
            self._show_info(f"Processing completed successfully!\n{files_processed} files processed.")
        else:
            self._show_warning(f"Processing completed with {files_failed} failures.\n{files_processed} files processed successfully.")
    ...
    def _save_settings(self) -> None:
        ...
        try:
            ...
            self.gui_settings.set_checkbox_state(
                self.tab_name, "write_moc_obsidian_pages", self.moc_obsidian_pages_checkbox.isChecked()
            )
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            self.moc_obsidian_pages_checkbox.setChecked(
                self.gui_settings.get_checkbox_state(self.tab_name, "write_moc_obsidian_pages", False)
            )
