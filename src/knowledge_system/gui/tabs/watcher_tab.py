...
class WatcherTab(BaseTab, FileOperationsMixin):
    ...
    def _start_watching(self):
        ...
        if not watch_path_obj.exists():
            self.show_error("Invalid Directory", f"Directory does not exist: {watch_path}")
            return
            
        if not watch_path_obj.is_dir():
            self.show_error("Invalid Directory", f"Path is not a directory: {watch_path}")
            return
    ...
    def _process_file(self, file_path: Path, config: Dict[str, Any]):
        ...
        try:
            ...
            if file_path.suffix.lower() in [".mp4", ".mp3", ".wav", ".webm", ".m4a", ".flac", ".ogg"]:
                try:
                    ...
                    if result.success:
                        ...
                        self.append_log(f"❌ Transcription failed: {'; '.join(result.errors)}")
            ...
            # Summarization for text files (automatic)
                ...
                try:
                    ...
                    if result.success:
                        ...
                        self.append_log(f"❌ Summarization failed: {'; '.join(result.errors)}")
    ...
    def _load_settings(self):
        ...
        try:
            ...
            self.auto_process_checkbox.setChecked(
                self.gui_settings.get_checkbox_state(self.tab_name, "auto_process", True)
            )
            ...
            # Load debounce delay
            saved_debounce = self.gui_settings.get_spinbox_value(self.tab_name, "debounce_delay", 5)
    ...
    def _save_settings(self):
        ...
        try:
            # Save line edit text
            self.gui_settings.set_line_edit_text(self.tab_name, "watch_directory", self.watch_directory.text())
            self.gui_settings.set_line_edit_text(self.tab_name, "file_patterns", self.file_patterns.text())
            
            # Save checkbox states
            self.gui_settings.set_checkbox_state(self.tab_name, "recursive", self.recursive_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "auto_process", self.auto_process_checkbox.isChecked())
            self.gui_settings.set_checkbox_state(self.tab_name, "dry_run", self.dry_run_checkbox.isChecked())
            
            # Save spinbox values
            self.gui_settings.set_spinbox_value(self.tab_name, "debounce_delay", self.debounce_delay.value())
