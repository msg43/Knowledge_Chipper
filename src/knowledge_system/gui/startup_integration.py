...
from ..config.process_isolation import (
...
class StartupIntegration:
        ...
        try:
            ...
            if issues:
                message_parts.extend(
                    ["Issues found:", *[f"• {issue}" for issue in issues], ""]
                )
            ...
            QMessageBox.warning(
                self.main_window, "System Requirements", "\n".join(message_parts)
            )
    ...
    def shutdown(self):
        ...
        try:
            ...
            if self.analytics and self.config:
                cleanup_days = self.config.get(
                    "checkpoint_system", "max_checkpoint_age_days", 7
                )
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        if not self.initialization_complete:
            return {"status": "initializing", "message": "System still initializing"}

        try:
            if self.analytics:
                return self.analytics.get_health_status()
            else:
                return {"status": "unknown", "message": "Analytics not available"}
        except Exception as e:
    ...
    def generate_diagnostics_report(self) -> str:
        ...
        try:
            ...
            if self.analytics:
                ...
                report_lines.extend(["Analytics Report:", analytics_report, ""])
