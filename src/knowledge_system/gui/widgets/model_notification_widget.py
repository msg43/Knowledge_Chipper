...
class ModelNotificationWidget(QFrame):
    ...
    def show_completion(self, model_type: str, success: bool, message: str):
        ...
        if model_type in self.model_states:
            self.model_states[model_type]['status'] = 'completed' if success else 'failed'
            self.model_states[model_type]['message'] = message
            self._update_display()
    ...
    def show_feature_blocked(self, feature: str, required_model: str):
        ...
        self.action_button.clicked.connect(lambda: self.retry_requested.emit(required_model))
    ...
    def _update_display(self):
        ...
        if active_downloads:
            avg_progress = total_progress / len(active_downloads) if active_downloads else 0
            ...
            self.icon_label.setText("❌")
            self.message_label.setText(
                f"Failed to download {len(failed)} model(s): "
                f"{', '.join(failed)}"
            )
