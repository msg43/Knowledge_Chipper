...
class ProcessingReport:
    ...
    def __init__(self, report_type: str, title: str, content: str, timestamp: Optional[datetime] = None):
    ...
    def save_to_file(self, output_dir: Union[str, Path]) -> Path:
...
class ReportManager:
    ...
    def add_report(self, report_type: str, title: str, content: str) -> ProcessingReport:
        """Add a new report."""
    ...
    def get_reports(self, report_type: Optional[str] = None) -> List[ProcessingReport]:
        """Get reports, optionally filtered by type."""
        if report_type:
            return [r for r in self.reports if r.type == report_type]
        return self.reports.copy()
    
    def get_latest_report(self, report_type: Optional[str] = None) -> Optional[ProcessingReport]:
        """Get the most recent report, optionally filtered by type."""
        reports = self.get_reports(report_type)
        return reports[-1] if reports else None
    
    def save_report(self, report: ProcessingReport, output_dir: Union[str, Path]) -> Path:
        """Save a report to file."""
        return report.save_to_file(output_dir)
    
    def clear_reports(self, report_type: Optional[str] = None):
    ...
    def generate_summary_report(self, include_types: Optional[List[str]] = None) -> ProcessingReport:
        ...
        if not reports:
            ...
            for report_type, type_reports in by_type.items():
                content_parts.append(f"### {report_type.title()} Reports ({len(type_reports)})\n")
