...
def upload(
    ...
    try:
        ...
        if filter_tier:
            ...
            claims_to_upload = [
                claim for claim in claims_to_upload 
                if claim.tier and claim.tier.upper() in allowed_tiers
            ]
            
            if not ctx.quiet:
                console.print(f"🔍 Filtered to tiers {', '.join(allowed_tiers)}: {len(claims_to_upload)} claims")
        ...
        if dry_run:
            console.print(f"[yellow]🔍 DRY RUN: Would upload {len(claims_to_upload)} claims:[/yellow]")
        ...
        # Upload via OAuth
        console.print(f"[blue]📤 Uploading {len(claims_to_upload)} claims to GetReceipts.org...[/blue]")
        ...
        console.print(f"[green]✅ Successfully uploaded {total_uploaded} records![/green]")
