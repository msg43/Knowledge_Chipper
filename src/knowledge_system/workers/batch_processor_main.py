...
class BatchProcessor:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.checkpoint_file = Path(args.checkpoint_file) if args.checkpoint_file else None
    ...
    def _audio_progress_callback(self, message: str, progress: Optional[int] = None):
    ...
    def _summarizer_progress_callback(self, message: str, progress: Optional[int] = None):
    ...
    def _moc_progress_callback(self, message: str, progress: Optional[int] = None):
        ...
        try:
            ...
            if self.config.get("transcribe") and self.audio_processor:
                ...
                if transcription_result.get("success"):
                    result["transcription_path"] = transcription_result.get("output_path")
                    self.ipc.send_progress(
                        current_file=self.files_processed + 1,
                        total_files=len(self.files),
                        stage="transcription",
                    ...
                    result["error"] = f"Transcription failed: {transcription_result.get('error')}"
                    return result
            
            # Summarization
            if (self.config.get("summarize") and 
                self.summarizer_processor and 
                result.get("transcription_path")):
                
                self.ipc.send_message("info", f"Summarizing {file_path_obj.name}")
                summary_result = self.summarizer_processor.process(result["transcription_path"])
                ...
                if summary_result.get("success"):
                    ...
                    result["error"] = f"Summarization failed: {summary_result.get('error')}"
                    return result
            
            # MOC Generation
            if (self.config.get("create_moc") and 
                self.moc_processor and 
                ...
                if moc_result.get("success"):
                    ...
                    result["error"] = f"MOC generation failed: {moc_result.get('error')}"
                    return result
    ...
    def run(self) -> int:
        ...
        try:
            self.ipc.send_message("info", f"Starting batch processing of {len(self.files)} files")
            
            # Check for existing checkpoint
            if self.tracker.has_checkpoint():
                checkpoint_data = self.tracker.load_checkpoint()
                completed_count = len(checkpoint_data.get("completed_files", []))
                self.ipc.send_message("info", f"Resuming from checkpoint - {completed_count} files already completed")
            ...
            for i, file_path in enumerate(self.files):
                ...
                if self._should_resume_from_checkpoint(file_path):
                    self.ipc.send_message("info", f"Skipping {Path(file_path).name} (already completed)")
                ...
                if result["success"]:
                    self.files_processed += 1
                    self.ipc.send_file_completed(file_path, True, "Processing completed successfully")
                else:
                    self.files_failed += 1
                    self.ipc.send_file_completed(file_path, False, result.get("error", "Unknown error"))
            ...
            if self.files_failed == 0:
                self.ipc.send_message("info", f"Batch processing completed successfully - {self.files_processed} files processed")
                return 0
            else:
                self.ipc.send_message("warning", f"Batch processing completed with {self.files_failed} failures")
                return 1
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Standalone batch processor for Knowledge Chipper")
...
def setup_logging(level: str):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.StreamHandler(sys.stderr)  # Send logs to stderr, keep stdout for IPC
        ]
    )
