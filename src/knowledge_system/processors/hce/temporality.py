...
from .types import ScoredClaim, TemporalityScore
...
class TemporalityAnalyzer:
    ...
    def analyze_claim_temporality(self, claim: ScoredClaim) -> tuple[TemporalityScore, float, str]:
        """
        ...
        try:
            ...
            evidence_text = "\n".join([ev.quote for ev in claim.evidence[:3]])  # Limit to first 3 pieces of evidence
            ...
            if result and isinstance(result, list) and len(result) > 0:
                ...
                confidence = min(max(analysis.get("confidence", 0.5), 0.0), 1.0)  # Clamp between 0-1
                ...
                if temporality_score not in [1, 2, 3, 4, 5]:
                    ...
                    confidence = 0.3
                    rationale = f"Invalid temporality score returned: {temporality_score}"
    ...
    def analyze_claims_batch(self, claims: List[ScoredClaim]) -> List[ScoredClaim]:
        ...
        """
        for claim in claims:
            temporality_score, confidence, rationale = self.analyze_claim_temporality(claim)
            ...
            logger.debug(f"Claim '{claim.canonical[:50]}...' classified as {temporality_score} (confidence: {confidence:.2f})")
...
def is_timeless_claim(claim_text: str) -> bool:
    ...
    timeless_indicators = [
        "always", "never", "fundamental", "principle", "law of", "theorem",
        "mathematical", "physical law", "human nature", "universal", "eternal",
        "inherent", "intrinsic", "by definition", "axiom"
    ]
...
def is_timely_claim(claim_text: str) -> bool:
    ...
    timely_indicators = [
        "next week", "this month", "current", "recent", "trending", "now",
        "today", "tomorrow", "this year", "breaking", "latest", "ongoing",
        "immediate", "short-term", "temporary", "will win", "will lose"
    ]
...
def estimate_claim_half_life(temporality: TemporalityType, claim_text: str) -> str:
    ...
    if temporality == "timeless":
        ...
        if any(term in claim_text.lower() for term in ["next week", "tomorrow", "today"]):
