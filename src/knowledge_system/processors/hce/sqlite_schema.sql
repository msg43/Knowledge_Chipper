...
CREATE TABLE IF NOT EXISTS episodes (
  ...
  source_id TEXT UNIQUE,              -- FK to existing videos table if available
  title TEXT,
...
CREATE TABLE IF NOT EXISTS segments (
  ...
  FOREIGN KEY (source_id) REFERENCES episodes(source_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS evidence_spans (
  ...
  FOREIGN KEY (source_id, claim_id) REFERENCES claims(source_id, claim_id) ON DELETE CASCADE,
  FOREIGN KEY (source_id, segment_id) REFERENCES segments(source_id, segment_id)
);
...
CREATE TABLE IF NOT EXISTS relations (
  ...
  FOREIGN KEY (source_id, source_claim_id) REFERENCES claims(source_id, claim_id) ON DELETE CASCADE,
  FOREIGN KEY (source_id, target_claim_id) REFERENCES claims(source_id, claim_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS people (
  ...
  PRIMARY KEY (source_id, mention_id),
  FOREIGN KEY (source_id) REFERENCES episodes(source_id) ON DELETE CASCADE
);
...
-- Performance indexes
CREATE INDEX IF NOT EXISTS idx_claims_episode_tier ON claims(source_id, tier);
...
CREATE INDEX IF NOT EXISTS idx_evidence_spans_segment ON evidence_spans(source_id, segment_id);
...
FROM claims c
LEFT JOIN evidence_spans e ON c.source_id = e.source_id AND c.claim_id = e.claim_id
