...
def find_conversational_boundaries(
    ...
    for i in range(quote_segment_idx - 1, -1, -1):
        ...
        if (seg.speaker != current_speaker and 
            current_speaker.lower() not in ['narrator', 'host', 'unknown']):
            ...
            if potential_duration + (seg_end - seg_start) <= target_context_seconds * 1.2:
    ...
    if method == "conversational_boundary":
        ...
        for seg in segments:
            ...
            if (seg_start is not None and seg_end is not None and 
                seg_start < context_end and seg_end > context_start):
                context_segments.append(seg)
        ...
        if quote_segment:
            ...
            context_start, context_end, context_text = quote_start, quote_end, evidence.quote
    else:
