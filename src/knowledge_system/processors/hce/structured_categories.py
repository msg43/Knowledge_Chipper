...
from .types import PipelineOutputs, StructuredCategory, ScoredClaim
...
class StructuredCategoryAnalyzer:
    ...
    def analyze_episode_categories(self, outputs: PipelineOutputs) -> List[StructuredCategory]:
        """
        ...
        try:
            ...
            prompt_text = self.template.format(
                ...
                claims_sample="\n".join([c.canonical for c in outputs.claims[:10]]),  # First 10 claims
                entities=", ".join(signals.get("entities", [])[:20]),  # First 20 entities
                concepts=", ".join(signals.get("concepts", [])[:15]),  # First 15 concepts
                jargon=", ".join(signals.get("jargon", [])[:15]),  # First 15 jargon terms
            ...
            for i, category_data in enumerate(result):
                try:
                    category = StructuredCategory(
                        ...
                        coverage_confidence=min(max(category_data.get("confidence", 0.5), 0.0), 1.0),
                        supporting_evidence=category_data.get("supporting_claim_ids", []),
                        frequency_score=category_data.get("frequency_score", 0.0)
                    )
                    categories.append(category)
                    
                except Exception as e:
                    logger.warning(f"Failed to parse category {i} for episode {outputs.episode_id}: {e}")
                    continue
            ...
            logger.error(f"Failed to analyze categories for episode {outputs.episode_id}: {e}")
            return []
    
    def propagate_categories_to_claims(self, claims: List[ScoredClaim], categories: List[StructuredCategory]) -> None:
        """
        ...
        try:
            ...
            for claim in claims:
                ...
                for category in categories:
                    relevance_score = self.calculate_claim_category_relevance(claim, category)
                ...
                logger.debug(f"Claim '{claim.canonical[:30]}...' assigned to {len(claim_categories)} categories")
                
        except Exception as e:
            logger.error(f"Failed to propagate categories to claims: {e}")
    
    def calculate_claim_category_relevance(self, claim: ScoredClaim, category: StructuredCategory) -> float:
        """
        ...
        try:
            ...
            keyword_matches = sum(1 for keyword in category_keywords if keyword in claim_text)
            keyword_score = keyword_matches / max(len(category_keywords), 1)
            
            # Check if this claim is explicitly listed as supporting evidence
            explicit_support = 1.0 if claim.claim_id in category.supporting_evidence else 0.0
    ...
    def calculate_frequency_scores(self, categories: List[StructuredCategory], signals: Dict[str, List[str]]) -> List[StructuredCategory]:
        """
...
def analyze_structured_categories(outputs: PipelineOutputs, model_uri: str) -> List[StructuredCategory]:
    """
    ...
    logger.info("Structured categories analysis disabled - no prompt template available")
    return []
...
def categorize_by_domain(entities: List[str], concepts: List[str], jargon: List[str]) -> Dict[str, float]:
    """
    ...
    """
    domain_keywords = {
        "technology": ["ai", "algorithm", "software", "computer", "digital", "data", "cloud", "api"],
        "science": ["research", "study", "experiment", "hypothesis", "theory", "evidence", "peer review"],
        "business": ["company", "market", "revenue", "profit", "strategy", "customer", "product"],
        "politics": ["government", "policy", "election", "democracy", "vote", "congress", "president"],
        "health": ["medical", "doctor", "patient", "treatment", "diagnosis", "therapy", "hospital"],
        "economics": ["economy", "inflation", "gdp", "market", "trade", "finance", "investment"],
        "education": ["school", "university", "student", "teacher", "learning", "curriculum", "degree"]
    }
    ...
    for domain, keywords in domain_keywords.items():
        score = sum(1 for term in all_terms if any(keyword in term for keyword in keywords))
