...
class PDFProcessor(BaseProcessor):
    ...
    def process(self, input_data: Any, dry_run: bool = False, **kwargs: Any) -> ProcessorResult:
        ...
        if isinstance(input_data, (str, Path)):
            path = Path(input_data)
            logger.info(f"PDF Processor received input: {path} (type: {type(input_data)})")
        ...
        logger.info(f"PDF Processor will process {len(paths)} files: {[str(p) for p in paths]}")
        ...
        for pdf_path in paths:
            ...
            logger.info(f"PDF file size: {pdf_path.stat().st_size if pdf_path.exists() else 'File not found'}")
            ...
            logger.info(f"Extracted text preview from {pdf_path.name}: {text_preview}...")
...
def fetch_pdf_text(pdf_path: Union[str, Path]) -> str:
