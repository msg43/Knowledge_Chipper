...
class WhisperCppTranscribeProcessor(BaseProcessor):
    ...
    def __init__(
        self, model: str = "base", use_coreml: Optional[bool] = None, progress_callback=None
    ):
    ...
    def _download_model(self, model_name: str, progress_callback=None) -> Path:
        ...
        if local_models_dir.exists():
            ...
            if local_model_path.exists():
                logger.info(
                    f"Using local whisper.cpp model: {local_model_path}")
    ...
    url, dest_path, reporthook=report_progress)
    ...
    def _validate_transcription_quality(self, text: str, audio_duration_seconds: Optional[float] = None) -> dict:
        ...
        if audio_duration_seconds and audio_duration_seconds > 30:  # Only for audio longer than 30 seconds
            ...
            elif words_per_minute < 40:  # 15-40 WPM suggests partial failure or mostly silence
                return {
                    "is_valid": False,
            ...
            elif words_per_minute > 300:  # >300 WPM suggests gibberish or processing error
                return {
                    "is_valid": False,
        ...
        if len(words) >= 10:
            ...
            if max_consecutive >= 5:
                return {"is_valid": False, "issue": f"Repetitive pattern detected ('{words[0]}' repeated {max_consecutive} times)"}
            ...
            if most_common_count / len(words) > 0.5:
                return {"is_valid": False, "issue": f"Single word '{most_common_word}' dominates transcription ({most_common_count}/{len(words)} occurrences)"}
        ...
        if single_char_words / len(words) > 0.3:
            return {"is_valid": False, "issue": f"Too many single-character words ({single_char_words}/{len(words)})"}
        
        # Check for likely foreign language when expecting English
        common_english_words = {'the', 'and', 'a', 'an', 'is', 'are', 'was', 'were', 'have', 'has', 'had', 'will', 'would', 'could', 'should', 'can', 'may', 'might', 'do', 'does', 'did', 'get', 'got', 'go', 'went', 'come', 'came', 'see', 'saw', 'know', 'think', 'want', 'like', 'time', 'people', 'way', 'day', 'man', 'woman', 'year', 'work', 'life', 'hand', 'part', 'place', 'case', 'point', 'government', 'company'}
        english_word_count = sum(1 for word in words if word in common_english_words)
        if len(words) > 50 and english_word_count / len(words) < 0.1:
            return {"is_valid": False, "issue": f"Very few common English words detected ({english_word_count}/{len(words)}), possible language mismatch"}
    ...
    def _get_audio_duration(self, input_path: Path) -> Optional[float]:
        """Get audio duration in seconds using ffprobe."""
        try:
            import subprocess
            probe_cmd = [
                "ffprobe", "-v", "quiet", "-show_entries", 
                "format=duration", "-of", "csv=p=0", str(input_path)
            ]
    ...
    def _process_audio(self, input_path: Path, **kwargs: Any) -> ProcessorResult:
        ...
        if audio_duration_seconds:
            logger.info(f"Audio duration: {audio_duration_seconds:.1f} seconds ({audio_duration_seconds/60:.1f} minutes)")
        ...
        try:
            ...
            for cmd_candidate in ["whisper-cli", "whisper-cpp", "whisper"]:
                try:
                    subprocess.run([cmd_candidate, "--help"], capture_output=True, check=True)
            ...
            if self.progress_callback:
                self.progress_callback(f"📥 Ensuring model '{self.model_name}' is available...", 30)
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if json_file.exists():
                    if self.progress_callback:
                        self.progress_callback("📄 Processing transcription results...", 85)
                    ...
                    try:
                        ...
                        logger.warning(f"JSON parsing failed: {e}, falling back to stdout")
                        ...
                        if self.progress_callback:
                            self.progress_callback("✅ Transcription text extracted from stdout", 100)
                    ...
                    if "transcription" in output_data:
                        if self.progress_callback:
                            self.progress_callback("🔍 Extracting transcription segments...", 90)
                        ...
                        if isinstance(transcription, list):
                            ...
                            logger.info(f"Joined {len(text_segments)} text segments, total length: {len(full_text)}")
                            segments = []
                            for segment in transcription:
                                ...
                                segments.append(
                                    {
                                        "start": (
                                            float(
                                                start_time) if start_time else 0.0
                                        ),
                            ...
                            segments = [
                                {"start": 0.0, "end": 0.0, "text": full_text}]
                    else:
                        # Try to extract from other possible formats
                        full_text = output_data.get(
                            "text", str(output_data)).strip()
                        segments = [
                            {"start": 0.0, "end": 0.0, "text": full_text}]
                    ...
                    validation_result = self._validate_transcription_quality(full_text, audio_duration_seconds)
                    if not validation_result["is_valid"]:
                        logger.warning(f"Transcription quality issue detected: {validation_result['issue']}")
                        if self.progress_callback:
                            self.progress_callback(f"⚠️ Quality issue: {validation_result['issue']}", 95)
                    
                    if self.progress_callback:
                        self.progress_callback(f"✅ Extracted {len(full_text):,} characters of transcription", 100)
                    ...
                    if self.progress_callback:
                        self.progress_callback(f"✅ Extracted {len(full_text):,} characters from stdout", 100)
    ...
    def _run_whisper_with_progress(self, cmd) -> subprocess.CompletedProcess:
        ...
        def monitor_progress():
            ...
            while process.poll() is None:
                ...
                try:
                    ...
                    probe_cmd = [
                        "ffprobe", "-v", "quiet", "-show_entries", 
                        "format=duration", "-of", "csv=p=0", cmd[cmd.index("-f") + 1]
                    ]
        ...
        if process.returncode != 0:
            ...
            if full_stderr:
                error_msg += f"\nStderr: {full_stderr}"
            raise subprocess.CalledProcessError(process.returncode, cmd, full_stdout, full_stderr)
    ...
    def _parse_whisper_output_for_progress(self, line: str, elapsed_time: float):
        ...
        if 'processing' in line or 'transcribing' in line:
            ...
            progress = 55 + min(20, int(elapsed_time / 2))  # Gradual increase from 55% to 75%
            self.progress_callback(f"🎯 {line.capitalize()}", progress)
        elif 'segment' in line or 'frame' in line:
            # Processing segments/frames
            progress = 60 + min(15, int(elapsed_time / 3))  # Gradual increase from 60% to 75%
...
def fetch_transcript(
    audio_path: Union[str, Path], model: str = "base", use_coreml: Optional[bool] = None
) -> Optional[str]:
