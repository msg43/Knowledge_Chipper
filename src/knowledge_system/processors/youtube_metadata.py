...
class YouTubeMetadata(BaseModel):
    ...
    caption_availability: Optional[bool] = Field(default=None, description="Whether captions are available")
    privacy_status: Optional[str] = Field(default=None, description="Privacy status")
    
    # Extraction metadata
    extraction_method: str = Field(default="yt-dlp", description="Method used to extract metadata")
    ...
    def to_markdown_metadata(self) -> str:
        ...
        lines.append(f"- **Extracted**: {self.fetched_at.strftime('%Y-%m-%d %H:%M:%S')}")
...
class YouTubeMetadataProcessor(BaseProcessor):
    ...
    def _extract_metadata(self, url: str) -> Optional[YouTubeMetadata]:
    ...
    def _write_simple_failure_log(self, errors: List[str]):
        ...
        try:
            ...
            with open(log_file, 'a', encoding='utf-8') as f:
                ...
                f.write(f"If failures persist, check proxy credentials and YouTube accessibility.\n")
                f.write(f"\n{'='*70}\n")
