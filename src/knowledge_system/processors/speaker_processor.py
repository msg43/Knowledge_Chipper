...
class SpeakerProcessor(BaseProcessor):
    ...
    def prepare_speaker_data(
        ...
        try:
            ...
            # 🎯 Note: Voice-based verification has been replaced by word-driven alignment
            # Speaker attribution is now handled by pyannote-whisper's diarize_text pattern
            # in audio_processor.py with median filtering for improved accuracy.
            # See: https://github.com/yinruiqing/pyannote-whisper
            if audio_path and len(speaker_map) >= 2 and word_timestamps:
                logger.info(
                    f"📝 Word-driven alignment already applied ({len(word_timestamps)} words) - "
                    f"skipping legacy verification"
                )
    ...
    # NOTE: The following deprecated methods have been removed as part of the
    # word-driven speaker alignment refactor:
    # - _split_mixed_speaker_segments() - replaced by pyannote-whisper word-driven alignment
    # - _reassign_segments_by_voice_verification() - replaced by median filter in audio_processor
    # - _verify_word_level_speakers() - replaced by pyannote-whisper diarize_text pattern
    #
    # Speaker attribution is now handled at the word level in audio_processor._merge_diarization()
    # using the pyannote-whisper pattern with median filtering for improved accuracy.
    # See: https://github.com/yinruiqing/pyannote-whisper
