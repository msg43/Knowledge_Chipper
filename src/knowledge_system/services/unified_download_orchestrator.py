...
class UnifiedDownloadOrchestrator:
    ...
    async def process_all(self) -> list[tuple[Path, str]]:
        ...
        """
        logger.info(f"🚀 Starting unified download orchestration for {len(self.youtube_urls)} URLs")
    ...
    async def _process_rss_downloads(
        ...
        if self.progress_callback:
            self.progress_callback(f"📥 Downloading from {len(rss_mappings)} podcast RSS feeds...")
    ...
    async def _process_youtube_downloads(
        self,
        urls_with_source_ids: dict[str, str]  # {youtube_url: source_id}
    ) -> list[tuple[Path, str]]:
        """
        ...
        logger.info(f"📺 Starting YouTube downloads for {len(urls_with_source_ids)} URLs")
    ...
    def _merge_download_queues(
        self,
        rss_files: list[tuple[Path, str]],
        youtube_files: list[tuple[Path, str]]
    ) -> list[tuple[Path, str]]:
        """
