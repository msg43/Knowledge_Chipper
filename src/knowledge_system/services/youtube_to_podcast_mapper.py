...
class YouTubeToPodcastMapper:
    ...
    def _query_podcast_index(self, channel_name: str) -> str | None:
        ...
        try:
            ...
            params = {"q": channel_name, "max": 5}  # Get top 5 results
    ...
    def _query_listen_notes(self, channel_name: str) -> str | None:
        ...
        try:
            ...
            headers = {"X-ListenAPI-Key": self.podcast_config.listen_notes_api_key}

            params = {"q": channel_name, "type": "podcast", "len_max": 5}
    ...
    def _query_itunes(self, channel_name: str) -> str | None:
        ...
        try:
            ...
            params = {"term": channel_name, "media": "podcast", "limit": 5}
        ...
        try:
            if self.cache_path.exists():
                with open(self.cache_path) as f:
                    cache = json.load(f)
                logger.info(
                    f"Loaded {len(cache)} cached mappings from {self.cache_path}"
                )
                return cache
        except Exception as e:
