...
class ClaimsUploadService:
    ...
    def _collect_associated_data(
        ...
        try:
            # Get episode data
            cursor.execute(
                "SELECT * FROM episodes WHERE episode_id = ?", (claim.episode_id,)
            )
            ...
            cursor.execute(
                "SELECT * FROM people WHERE episode_id = ?", (claim.episode_id,)
            )
            claim.people = [dict(row) for row in cursor.fetchall()]

            # Get concepts from this episode
            cursor.execute(
                "SELECT * FROM concepts WHERE episode_id = ?", (claim.episode_id,)
            )
            claim.concepts = [dict(row) for row in cursor.fetchall()]

            # Get jargon from this episode
            cursor.execute(
                "SELECT * FROM jargon WHERE episode_id = ?", (claim.episode_id,)
            )
