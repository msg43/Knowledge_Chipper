...
class SupabaseSyncService:
    ...
    def sync_all(
        ...
        if not self.is_configured():
            return SyncResult(
                success=False, errors=[{"error": "Supabase not configured"}]
            )
    ...
    def sync_table(
        ...
        try:
            ...
            result.errors.append({"table": table_name, "error": str(e)})

        return result

    def _pull_table(
        self, table_name: str, conflict_resolution: ConflictResolution
    ) -> SyncResult:
        ...
        try:
            ...
            with self.db.get_session() as session:
                for remote_record in remote_records:
                    ...
                    if local_record:
                        ...
                        local_checksum = local_data.get(
                            "sync_checksum"
                        ) or self.calculate_checksum(local_data)
                        remote_checksum = remote_record.get(
                            "sync_checksum"
                        ) or self.calculate_checksum(remote_record)
                        ...
                        if local_checksum != remote_checksum:
                            ...
                            conflict = SyncConflict(
                                ...
                                local_updated=local_data.get(
                                    "updated_at", datetime.now()
                                ),
                                remote_updated=datetime.fromisoformat(
                                    remote_record.get("updated_at")
                                ),
                            ...
                            if conflict_resolution == ConflictResolution.REMOTE_WINS:
                                self._update_local_record(
                                    session, table_name, record_id, remote_record
                                )
            ...
            result.errors.append(
                {"operation": "pull", "table": table_name, "error": str(e)}
            )
        ...
        try:
            with self.db.get_session() as session:
                ...
                for record in pending_records:
                    ...
                    clean_data = {
                        k: v for k, v in record_data.items() if not k.startswith("_")
                    }
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error pushing record {record_id} in {table_name}: {e}"
                        )
            ...
            result.success = False
            result.errors.append(
                {"operation": "push", "table": table_name, "error": str(e)}
            )
    ...
    def _get_last_sync_time(self, table_name: str) -> datetime | None:
    ...
    def _update_local_record(
        self, session: Session, table_name: str, record_id: str, data: dict[str, Any]
    ):
    ...
    def _insert_local_record(
        self, session: Session, table_name: str, data: dict[str, Any]
    ):
    ...
    def resolve_conflict(
        self, conflict: SyncConflict, resolution: ConflictResolution
    ) -> bool:
            ...
            with self.db.get_session() as session:
                if resolution == ConflictResolution.LOCAL_WINS:
                    ...
                    self._push_single_record(
                        conflict.table_name, conflict.record_id, conflict.local_data
                    )
                    ...
                    # For now, just log
                    logger.info(
                        f"Merge resolution not implemented for {conflict.table_name}:{conflict.record_id}"
                    )
                    return False
    ...
    def _push_single_record(
        ...
        # Push to Supabase
        self.client.table(table_name).upsert(clean_data, on_conflict=pk_field).execute()
