...
class MultiAccountDownloadScheduler:
        ...
        try:
            ...
            if result["success"]:
                ...
                if is_auth_error:
                    ...
                    # Non-auth error
                    logger.warning(
                        f"Download failed (account {account_idx+1}): {error_msg}"
                    )
    ...
    async def _handle_auth_failure(self, account_idx: int, failed_url: str):
        ...
        if health["consecutive_auth_failures"] >= 3:
            ...
            logger.error(f"   Likely cause: Stale cookies (YouTube session expired)")
            logger.info(
    ...
    async def download_batch_with_rotation(
        ...
        unique_urls, duplicate_results = self.dedup_service.check_batch_duplicates(
            urls, DuplicationPolicy.SKIP_ALL
        )
        ...
        logger.info(f"🚀 Starting downloads with {len(self.schedulers)} account(s)")
        ...
        for idx, url in enumerate(unique_urls, 1):
            ...
            while account_info is None:
                ...
                if (
                    processing_queue
                    and processing_queue.qsize() >= self.parallel_workers * 2
                ):
                    account_info = None
