...
class ProcessIsolationConfig:
    ...
    def _load_system_defaults(self):
        ...
        if memory_gb < 4:
            ...
            # Default system
            self.config["resource_limits"]["max_memory_per_process_gb"] = memory_gb * 0.8
    ...
    def _load_environment_config(self):
        ...
        env_mappings = {
            ...
            "KC_MEMORY_THRESHOLD": ("memory_management", "memory_pressure_threshold", float),
            "KC_MAX_RESTART_ATTEMPTS": ("process_isolation", "max_restart_attempts", int),
            "KC_CHECKPOINT_ENABLED": ("checkpoint_system", "enabled", bool),
            "KC_AUTO_RECOVERY": ("safety_features", "auto_recovery_enabled", bool),
            "KC_MEMORY_LIMIT_GB": ("resource_limits", "max_memory_per_process_gb", float),
            "KC_FALLBACK_TO_THREADS": ("compatibility", "fallback_to_thread_mode", bool)
        }
    ...
    def _load_config_file(self):
        ...
        if not self.config_file:
            ...
            possible_locations = [
                ...
                Path(__file__).parent.parent.parent.parent / "config" / "process_isolation.yaml"
            ]
    ...
    def should_fallback_to_threads(self) -> bool:
        ...
        if platform_name not in ["Linux", "Darwin", "Windows"]:
            logger.warning(f"Platform {platform_name} may not fully support process isolation")
    ...
    def get_memory_limits(self) -> Dict[str, float]:
        ...
        return {
            "pressure_threshold": self.get("memory_management", "memory_pressure_threshold", 85.0),
            "swap_threshold": self.get("memory_management", "swap_pressure_threshold", 50.0),
            "growth_rate_threshold": self.get("memory_management", "growth_rate_threshold", 10.0),
            "max_process_memory_gb": self.get("resource_limits", "max_memory_per_process_gb"),
            "cache_limit_gb": self.get("memory_management", "model_cache_limit_gb", 4.0)
        }
    ...
    def get_checkpoint_config(self) -> Dict[str, Any]:
        ...
        if not config.get("checkpoint_directory"):
            checkpoint_dir = Path.home() / ".cache" / "knowledge_chipper" / "checkpoints"
    ...
    def get_restart_policy(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "backoff_seconds": self.get("process_isolation", "restart_backoff_seconds", [5, 10, 30]),
    ...
    def save_config(self, file_path: Optional[str] = None):
        ...
        if not save_path:
            save_path = Path.home() / ".config" / "knowledge_chipper" / "process_isolation.yaml"
    ...
    def validate_config(self) -> List[str]:
        ...
        if not 10 <= memory_threshold <= 95:
            issues.append(f"Memory pressure threshold {memory_threshold}% is outside safe range (10-95%)")
        
        # Check restart attempts
        max_attempts = self.get("process_isolation", "max_restart_attempts")
        if not 0 <= max_attempts <= 10:
            issues.append(f"Max restart attempts {max_attempts} is outside reasonable range (0-10)")
...
def get_process_isolation_config(config_file: Optional[str] = None) -> ProcessIsolationConfig:
...
def validate_system_requirements() -> Dict[str, Any]:
    ...
    try:
        ...
        if memory_gb < requirements["min_memory_gb"]:
            ...
            results["issues"].append(f"Insufficient memory: {memory_gb:.1f}GB < {requirements['min_memory_gb']}GB")
        ...
        if python_version[:2] < min_version:
            results["meets_requirements"] = False
            results["issues"].append(f"Python version {python_version[0]}.{python_version[1]} < {requirements['min_python_version']}")
