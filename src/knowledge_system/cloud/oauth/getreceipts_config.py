...
PRODUCTION = {
    'base_url': 'https://skipthepodcast.com',
    'supabase_url': 'https://sdkxuiqcwlmbpjvjdpkj.supabase.co',
    'supabase_anon_key': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNka3h1aXFjd2xtYnBqdmpkcGtqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU4MTU3MzQsImV4cCI6MjA3MTM5MTczNH0.VoP6yX3GwyVjylgioTGchQYwPQ_K2xQFdHP5ani0vts',
...
def validate_config(config: Dict[str, Any]) -> bool:
    ...
    for field in required_fields:
        if not config.get(field) or config[field] in ['YOUR-PROJECT.supabase.co', 'YOUR-SUPABASE-ANON-KEY-HERE']:
...
def print_config():
    ...
    print(f"  Anon Key: {config['supabase_anon_key'][:20]}..." if config['supabase_anon_key'] else "  Anon Key: NOT SET")
...
if __name__ == "__main__":
    ...
    config = get_config()
    if not validate_config(config):
        print("⚠️  GetReceipts configuration needs setup. Run 'python getreceipts_config.py' for instructions.")
