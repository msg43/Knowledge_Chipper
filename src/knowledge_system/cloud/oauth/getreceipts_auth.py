...
class GetReceiptsAuth:
    ...
    def authenticate(self) -> dict[str, Any]:
        ...
        This method:
        1. Checks if OAuth endpoint is available
        2. Starts a local callback server
        3. Opens browser to GetReceipts.org OAuth page
        4. Waits for user to complete authentication
        5. Captures and returns authentication result
        ...
        oauth_url = (
            f"{self.base_url}/auth/signin"  # Updated to match documentation pattern
        )
        ...
        try:
            ...
            if response.status_code == 404:
                raise Exception(
                    "❌ OAuth authentication is not yet available on skipthepodcast.com.\n\n"
                    "🔧 The OAuth endpoints haven't been implemented yet. Please contact the "
                    "skipthepodcast.com team to enable OAuth authentication for Knowledge_Chipper.\n\n"
                    "📋 They need to implement the endpoints described in:\n"
                    "docs/SKIPTHEPODCAST_OAUTH_IMPLEMENTATION.md\n\n"
                    "🎯 Expected endpoint: /auth/signin\n"
                    f"🌐 Checked URL: {oauth_url}"
                )
        except requests.exceptions.RequestException as e:
            print(f"⚠️  Warning: Could not verify OAuth endpoint (network error: {e})")
            print("🔄 Proceeding anyway in case it's a temporary network issue...")
        ...
        while (
            ...
            time.sleep(0.5)  # Shorter sleep for more responsive cancellation
    ...
    def _start_callback_server(self, result_dict: dict):
        ...
        try:
            # Try to bind to the port first to check availability
            ...
            try:
                ...
                result_dict[
                    "error"
                ] = f"Port {self.callback_port} is already in use. Please close other applications using this port."
                return

            server = HTTPServer(("localhost", self.callback_port), CallbackHandler)
            server.timeout = 1

            # Handle requests with timeout and result checking
            max_iterations = 300  # 5 minutes with 1-second timeouts
            iterations = 0

            while not result_dict and iterations < max_iterations:
                # Check for cancellation from main thread
            ...
            # If we timed out without getting a result, set error
            if not result_dict and iterations >= max_iterations:
                result_dict["error"] = "Callback server timeout - no response received"
    ...
    def cancel_authentication(self):
        ...
        This can be called from another thread to interrupt the authentication flow.
        """
