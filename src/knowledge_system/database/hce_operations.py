    ...
    with db_service.get_session() as session:
        ...
        for output in miner_outputs:
            ...
            for claim_data in output.claims:
                ...
                claim_id = claim_data.get("claim_id") or \
                           claim_data.get("candidate_id") or \
                           f"claim_{uuid.uuid4().hex[:8]}"
                
                # Check if claim already exists
                existing_claim = session.query(Claim).filter_by(
                    episode_id=episode_id,
                    claim_id=claim_id
                ).first()
                ...
                if not existing_claim:
                    claim = Claim(
                        episode_id=episode_id,
                        claim_id=claim_id,
                        canonical=claim_data.get("claim_text") or claim_data.get("text", ""),
                        ...
                        first_mention_ts=claim_data.get("timestamp") or claim_data.get("t0"),
            ...
            for jargon_data in output.jargon:
                ...
                existing_jargon = session.query(Jargon).filter_by(
                    episode_id=episode_id,
                    term_id=term_id
                ).first()
                ...
                if not existing_jargon:
                    jargon = Jargon(
                        ...
                        first_mention_ts=jargon_data.get("timestamp") or jargon_data.get("t0"),
                    )
                    session.add(jargon)
            
            # Store people
            for person_data in output.people:
                person_id = person_data.get("person_id") or \
                           person_data.get("mention_id") or \
                           f"person_{uuid.uuid4().hex[:8]}"
                ...
                if not existing_person:
                    person = Person(
                        ...
                        description=person_data.get("description") or person_data.get("normalized"),
                        first_mention_ts=person_data.get("timestamp") or person_data.get("t0"),
                    )
                    session.add(person)
            ...
            for model_data in output.mental_models:
                model_id = model_data.get("model_id") or \
                          model_data.get("concept_id") or \
                          f"concept_{uuid.uuid4().hex[:8]}"
                
                existing_concept = session.query(Concept).filter_by(
                    episode_id=episode_id,
                    concept_id=model_id
                ).first()
...
def load_mining_results(
    ...
    with db_service.get_session() as session:
        ...
        logger.info(f"Loaded mining results for episode {episode_id}: "
...
def store_transcript(
    ...
    with db_service.get_session() as session:
        ...
        session.commit()
        logger.info(f"Stored transcript reference for episode {episode_id}: {transcript_path}")
...
def get_episode_summary(
    ...
    with db_service.get_session() as session:
        ...
        return {
            ...
            "total_extractions": claims_count + jargon_count + people_count + concepts_count,
        }
