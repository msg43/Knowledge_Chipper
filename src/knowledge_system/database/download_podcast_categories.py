...
class PodcastCategoryDownloader:
    ...
    def query_sparql(self, query: str, query_type: str) -> List[Dict]:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                logger.info(f"  Attempt {attempt}/{self.max_retries} for {query_type}...")
    ...
    def _parse_results(self, data: Dict) -> List[Dict]:
        ...
        for binding in data.get('results', {}).get('bindings', []):
            ...
            # Get label
            label = binding.get('genreLabel', binding.get('topicLabel', {})).get('value', '')
    ...
    def _convert_to_vocabulary(self, categories: List[Dict]) -> Dict:
        ...
        return {
            'metadata': {
                ...
                'from_genres': len([c for c in categories if c['source'] == 'P136_genre']),
                'from_topics': len([c for c in categories if c['source'] == 'P921_topic']),
            },
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Download podcast categories from WikiData')
