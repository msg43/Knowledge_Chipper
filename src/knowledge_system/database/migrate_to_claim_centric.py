...
def migrate_to_claim_centric(old_db_path: Path, new_db_path: Path) -> None:
    ...
    try:
        ...
        old_sources = old_session.execute(
            text(
                """
            SELECT
        ...
        old_episodes = old_session.execute(
            text(
                """
        ...
        old_segments = old_session.execute(
            text(
                """
        ...
        old_claims = old_session.execute(
            text(
                """
            SELECT
        ...
        old_evidence = old_session.execute(
            text(
                """
            SELECT
        ...
        old_relations = old_session.execute(
            text(
                """
        ...
        old_people = old_session.execute(
            text(
                """
            SELECT
        ...
        old_concepts = old_session.execute(
            text(
                """
            SELECT
        ...
        old_jargon = old_session.execute(
            text(
                """
            SELECT
        ...
        logger.info(
            "  1. Load WikiData vocabulary: python -m src.knowledge_system.database.load_wikidata_vocab"
        )
