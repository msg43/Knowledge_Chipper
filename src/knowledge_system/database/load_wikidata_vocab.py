...
def load_wikidata_vocabulary(session: Session, seed_file: Path | None = None) -> int:
    ...
    for cat_data in categories:
        ...
        existing = (
            session.query(WikiDataCategory).filter_by(wikidata_id=wikidata_id).first()
        )
        ...
        if existing:
            ...
            logger.debug(
                f"Updated WikiData category: {wikidata_id} ({cat_data['category_name']})"
            )
        else:
            # Create new category
            ...
            loaded_count += 1
            logger.debug(
                f"Created WikiData category: {wikidata_id} ({cat_data['category_name']})"
            )
        ...
        if aliases:
            ...
            for alias in aliases:
                alias_obj = WikiDataAlias(wikidata_id=wikidata_id, alias=alias)
                session.add(alias_obj)

    session.commit()

    logger.info(
        f"✅ Loaded {loaded_count} new WikiData categories, updated {len(categories) - loaded_count}"
    )
    return loaded_count
