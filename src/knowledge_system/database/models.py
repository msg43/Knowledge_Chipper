...
class MediaSource(Base):
    ...
    segments = relationship(
        "Segment", back_populates="source", cascade="all, delete-orphan"
    )
    ...
    transcripts = relationship(
        "Transcript", back_populates="source", cascade="all, delete-orphan"
    )
    summaries = relationship(
        "Summary", back_populates="source", cascade="all, delete-orphan"
    )
    moc_extractions = relationship(
        "MOCExtraction", back_populates="source", cascade="all, delete-orphan"
    )
    generated_files = relationship(
        "GeneratedFile", back_populates="source", cascade="all, delete-orphan"
    )
...
class Segment(Base):
    ...
    # Relationships
    source = relationship("MediaSource", back_populates="segments")
    evidence_spans = relationship("EvidenceSpan", back_populates="segment")


class Transcript(Base):
    """Transcript data with support for multiple versions per source."""
    ...
    # Relationships
    source = relationship("MediaSource", back_populates="transcripts")
    ...
    def __repr__(self) -> str:
        return f"<Transcript(transcript_id='{self.transcript_id}', source_id='{self.source_id}', language='{self.language}')>"


class Summary(Base):
    """Summaries with different models/templates per source."""
    ...
    def __repr__(self) -> str:
        return f"<Summary(summary_id='{self.summary_id}', source_id='{self.source_id}', llm_model='{self.llm_model}')>"
...
class MOCExtraction(Base):
    ...
    # Relationships
    source = relationship("MediaSource", back_populates="moc_extractions")
    ...
    def __repr__(self) -> str:
        return f"<MOCExtraction(moc_id='{self.moc_id}', source_id='{self.source_id}', theme='{self.theme}')>"
...
class GeneratedFile(Base):
    ...
    # Relationships
    source = relationship("MediaSource", back_populates="generated_files")
...
class BrightDataSession(Base):
    ...
    # Relationships
    source = relationship("MediaSource", back_populates="bright_data_sessions")
