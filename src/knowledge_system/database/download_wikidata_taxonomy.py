...
class WikiDataTaxonomyDownloader:
    ...
    def query_sparql(self, query: str, max_retries: int = 5) -> list[dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"SPARQL query attempt {attempt + 1}/{max_retries} failed: {e}"
                )
    ...
    def download_subcategories(
        self, parent_ids: list[str], depth: int = 2
    ) -> list[dict]:
        """
        ...
        for batch_start in range(0, len(parent_ids), 50):  # Batch by 50
            ...
            try:
                ...
                logger.info(
                    f"  Batch {batch_start//50 + 1}: {len(parsed)} subcategories"
                )
    ...
    def download_complete_taxonomy(
        ...
        if len(unique_categories) > max_total:
            ...
            logger.info(
                f"  Trimmed to {max_total}: {len(general_cats)} general + {len(specific_cats)} specific"
            )
        ...
        vocabulary = {
            ...
            "metadata": {
                ...
                "general_categories": len(
                    [c for c in unique_categories if c["level"] == "general"]
                ),
                "specific_categories": len(
                    [c for c in unique_categories if c["level"] == "specific"]
                ),
    ...
    def save_vocabulary(self, vocabulary: dict) -> None:
        ...
        logger.info(
            f"   Total categories: {vocabulary['metadata']['total_categories']}"
        )
...
def download_wikidata_vocabulary(
    ...
    return downloader.download_and_save(
        max_total=max_categories, subcategory_depth=subcategory_depth
    )


def merge_with_existing(
    new_vocab_file: Path, existing_vocab_file: Path, output_file: Path
) -> dict:
    """
    ...
    merged = {
        ...
        "metadata": {
            ...
            "general_categories": len(
                [c for c in merged_categories if c["level"] == "general"]
            ),
            "specific_categories": len(
                [c for c in merged_categories if c["level"] == "specific"]
            ),
        },
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--depth", "-d", type=int, default=2, help="Subcategory depth: 1-3 (default: 2)"
    )
    parser.add_argument(
    ...
    try:
        ...
        print(
            f'     python -c "from src.knowledge_system.services.wikidata_categorizer import WikiDataCategorizer; '
        )
