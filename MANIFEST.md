...
**Last Updated:** November 1, 2025 (YouTube-to-RSS Migration System)
...
**Solution:** ✅ IMPLEMENTED intelligent YouTube-to-podcast-RSS mapping with parallel processing  
**Architecture:** Map YouTube URLs → Podcast RSS feeds → Download from RSS (no rate limiting) + YouTube fallback  
...
1. **YouTubeToPodcastMapper** (`src/knowledge_system/services/youtube_to_podcast_mapper.py`)
   - Multi-API discovery: PodcastIndex → ListenNotes → iTunes
...
**Resolution:** ✅ IMPLEMENTED comprehensive session-based download strategy  
**Strategy Components:**
...
**Findings:**
- ✅ **Audio Transcription (YouTube)** - FIXED: Was creating new record instead of using existing YouTube metadata
- ✅ **Document Processing** - FIXED: Now uses deterministic path-based hash instead of timestamp
- ⚠️ **HCE Summarization** - May create fallback record if wrong video_id passed (needs investigation)
- ✅ **HCE Episode Storage** - Uses proper UPSERT pattern (good pattern, no changes needed)
- ⚠️ **Transcription Retry** - May create duplicate on retry (minor fix needed)
...
- ✅ **ID Passing Confirmed** - Process Tab passes filename stem instead of source_id to summarization, causing duplicates
- ✅ **ID Naming Analysis** - Found multiple names (video_id, media_id, source_id) used for same concept
- ✅ **Unification Implemented** - Eliminated Episode table, renamed all video_id to source_id, standardized all code
- ✅ **Core Complete** - Database schema rebuilt, 27 bulk renames applied, source_id in transcript YAML
...
**Resolution:** ✅ UNIFIED to single code path using multi-account infrastructure  
...
**Resolution:** ✅ REMOVED phantom checkboxes and all associated code  
...
- **Settings Button Updated** - "🎤 Edit Speaker Mappings" button now opens CSV file instead of YAML
...
- **Priority Order** - Format 139 (3-5MB) → other audio-only (10-20MB) → worst video (20-40MB) → best video (last resort)
