...
def debug_transcript_availability(video_url: str):
    ...
    if not username:
        ...
        print(f"✅ WebShare username: {username[:4]}...{username[-4:] if len(username) > 8 else username}")
        ...
        if not available_transcripts:
            ...
            for transcript in transcript_list:
                try:
                    ...
                    if transcript_data:
                        print(f"✅ Successfully fetched {lang_code} transcript ({len(transcript_data)} segments)")
