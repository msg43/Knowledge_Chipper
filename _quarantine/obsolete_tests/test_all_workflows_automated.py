...
class TestAllGUIWorkflows:
    ...
    def test_youtube_download_workflow(self, gui_tester):
        ...
        assert gui_tester.switch_to_tab(
            "Transcribe"
        ...
        gui_tester.record_result(
            "youtube_download_workflow", result, "YouTube URL input via Transcribe tab"
        )
    ...
    def test_transcription_workflow(self, gui_tester):
        ...
        gui_tester.record_result(
            "transcription_workflow", result, "Transcribe tab UI elements accessible"
        )
    ...
    def test_summarization_workflow(self, gui_tester):
        ...
        assert gui_tester.switch_to_tab(
            "Summarize"
        ), "Failed to switch to Summarize tab"

        # In testing mode, just verify tab is accessible
        result = True  # Tab loaded successfully

        gui_tester.record_result(
            "summarization_workflow", result, "Summarize tab UI elements accessible"
        )
    ...
    def test_prompts_workflow(self, gui_tester):
        ...
        gui_tester.record_result("prompts_workflow", result, "Prompts tab accessible")
    ...
    def test_review_tab_database(self, gui_tester):
        ...
        gui_tester.record_result(
            "review_tab_database", result, "Review tab database integration"
        )
    ...
    def test_settings_configuration(self, gui_tester):
        ...
        gui_tester.record_result(
            "settings_configuration", result, "Settings tab accessible"
        )
    ...
    def test_introduction_tab(self, gui_tester):
        ...
        assert gui_tester.switch_to_tab(
            "Introduction"
        ), "Failed to switch to Introduction tab"

        # In testing mode, just verify tab is accessible
        result = True  # Tab loaded successfully

        gui_tester.record_result(
            "introduction_tab", result, "Introduction tab accessible"
        )
    ...
    def test_settings_persistence(self, gui_tester):
        ...
        gui_tester.record_result(
            "settings_persistence", result, "Settings tab is functional"
        )
    ...
    def test_all_tabs_load(self, gui_tester):
        ...
        if not tabs:
            gui_tester.record_result(
                "all_tabs_load", False, "Could not find tabs widget"
            )
        ...
        details = f"Tested {tab_count} tabs" + (
            f", {len(failed_tabs)} failed: {failed_tabs}" if failed_tabs else ""
        )
    ...
    def test_concurrent_operations(self, gui_tester):
        ...
        gui_tester.record_result(
            "concurrent_operations", result, "Monitor tab handles job queue display"
        )
    ...
    def test_cleanup_and_exit(self, gui_tester):
        ...
        gui_tester.record_result(
            "cleanup_and_exit", result, "Application exits cleanly"
        )
