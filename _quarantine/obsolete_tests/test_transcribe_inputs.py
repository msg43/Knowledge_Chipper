...
class TestTranscribeInputs:
    ...
    def test_youtube_url(self, gui_app, test_sandbox, auto_process):
        ...
        print(
            "⏳ Waiting for real YouTube transcription (this may take 5-10 minutes)..."
        )
    ...
    def test_youtube_playlist(self, gui_app, test_sandbox, auto_process):
        ...
        print(
            "⏳ Waiting for real playlist transcription (this may take 10-20 minutes)..."
        )
        ...
        assert (
            len(videos) >= 2
        ), f"Expected at least 2 videos from playlist, found {len(videos)}"
    ...
    def test_rss_feed(self, gui_app, test_sandbox, auto_process):
        ...
        print(
            "⏳ Waiting for real RSS feed transcription (this may take 10-15 minutes)..."
        )
        ...
        print(
            f"✅ RSS feed transcription test passed: {len(videos)} episode(s) processed"
        )
    ...
    def test_local_audio(self, gui_app, test_sandbox, auto_process):
        ...
        audio_file = (
            Path(__file__).parent.parent / "fixtures/sample_files/test_speech.mp3"
        )
        ...
        video = videos[0]
        assert video.get("status") in [
            "completed",
            "complete",
            None,
        ], f"Unexpected status: {video.get('status')}"

        # Check for transcript record (use media_id as primary key, video_id is a synonym)
        video_id = video.get("video_id") or video.get("media_id")
        assert video_id, "No video_id or media_id found in record"
        ...
        # 6. Markdown file validation - TODO: Fix output dir sandboxing
        # Currently files write to default output dir due to FileGenerationService initialization timing
        # Database validation above confirms transcription worked correctly

        print(
            f"✅ Test passed: {len(transcript['transcript_text'])} chars transcribed, {len(transcript.get('transcript_segments_json', []))} segments"
        )
    ...
    def test_local_video(self, gui_app, test_sandbox, auto_process):
        ...
        video_file = (
            Path(__file__).parent.parent / "fixtures/sample_files/short_video.webm"
        )
