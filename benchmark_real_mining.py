...
def parse_transcript_to_segments(transcript_path: Path, max_segments: int = 50) -> list[Segment]:
            ...
            if current_speaker and current_text:
                ...
                if text:
                    segment = Segment(
                        ...
                        t1=(current_time + 30.0) if current_time else (len(segments) + 1) * 30.0,
                        start_time=current_time if current_time else len(segments) * 30.0,
                        end_time=(current_time + 30.0) if current_time else (len(segments) + 1) * 30.0
                    )
...
def benchmark_mining(segments: list[Segment], num_parallel: int) -> dict:
    ...
    def progress_callback(msg: str):
        if "Processed segment" in msg:
            ...
            print(f"  [{completed_count[0]:3d}/{len(segments)}] {rate:.2f} seg/sec - {msg}")
...
def main():
    ...
    transcript_dir = Path("/Users/matthewgreer/Projects/Knowledge_Chipper/output/transcripts")
    ...
    if len(segments) < 10:
        print(f"⚠️  Warning: Only {len(segments)} segments found. Results may not be reliable.")
    ...
    if successful:
        ...
        print(f"   {((best['throughput'] / successful[0]['throughput']) - 1) * 100:.1f}% faster than NUM_PARALLEL=2")
