...
def parse_relation(node: dict) -> Optional[str]:
    ...
    def visit(node: dict) -> None:
        ...
        if node_type == "Seq Scan" and relation and node.get("Filter"):
            ...
            for expr in cols:
                ...
                suggestions.append(Suggestion(ddl=ddl, reason=f"Seq Scan filter: {filt}"))
                ...
                for expr in cols:
                    ...
                    if '=' in cond and '.' in expr:
                        ...
                        if len(parts) >= 2:
                            ...
                            if relation:
                                ddl = suggest_index_for_expression(relation, expr)
                                suggestions.append(Suggestion(ddl=ddl, reason=f"Join condition: {cond}"))
        ...
        if node_type == "Sort" and node.get("Sort Key"):
            ...
            if relation and rows and rows > 10000:
                ...
                suggestions.append(Suggestion(ddl=ddl, reason=f"Sort on {sort_keys} over {rows} rows"))
