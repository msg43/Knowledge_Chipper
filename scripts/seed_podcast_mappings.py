...
def main():
    parser = argparse.ArgumentParser(
        description="Seed podcast-to-host mappings database"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview changes without making them"
    )
    parser.add_argument(
        "--force", action="store_true", help="Overwrite existing mappings"
    )
    parser.add_argument(
        "--limit", type=int, help="Limit number of mappings to add (for testing)"
    )
    ...
    if args.dry_run:
        ...
        print()
        print(
            "🔧 Your speaker mapping database now includes 300+ popular podcast hosts."
        )
    ...
    print(
        "   Knowledge Chipper → 🎙️ Speaker Attribution tab → Channel-to-Host Mappings"
    )
