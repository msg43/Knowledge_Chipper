...
def test_context_quotes_sql():
    ...
    # Check jargon
    cursor.execute("SELECT COUNT(*) FROM jargon WHERE context_quote IS NOT NULL AND context_quote != ''")
    ...
    if total_jargon > 0:
        ...
        # Show sample
        cursor.execute("SELECT term, definition, context_quote FROM jargon WHERE context_quote IS NOT NULL AND context_quote != '' LIMIT 3")
        samples = cursor.fetchall()
        if samples:
            print(f"  Sample entries:")
            for term, definition, quote in samples:
                print(f"    - {term}: {quote[:60]}..." if len(quote) > 60 else f"    - {term}: {quote}")
    
    # Check people
    cursor.execute("SELECT COUNT(*) FROM people WHERE context_quote IS NOT NULL AND context_quote != ''")
    ...
    if total_people > 0:
        ...
        # Show sample
        cursor.execute("SELECT name, description, context_quote FROM people WHERE context_quote IS NOT NULL AND context_quote != '' LIMIT 3")
        samples = cursor.fetchall()
        if samples:
            print(f"  Sample entries:")
            for name, description, quote in samples:
                quote_str = quote if quote else "(none)"
                print(f"    - {name}: {quote_str[:60]}..." if len(quote_str) > 60 else f"    - {name}: {quote_str}")
    
    # Check concepts
    cursor.execute("SELECT COUNT(*) FROM concepts WHERE context_quote IS NOT NULL AND context_quote != ''")
    ...
    if total_concepts > 0:
        ...
        # Show sample
        cursor.execute("SELECT name, description, context_quote FROM concepts WHERE context_quote IS NOT NULL AND context_quote != '' LIMIT 3")
        samples = cursor.fetchall()
        if samples:
            print(f"  Sample entries:")
            for name, description, quote in samples:
