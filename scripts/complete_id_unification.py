...
def replace_in_file(file_path: Path, replacements: list[tuple[str, str]]) -> int:
    ...
    try:
        ...
        if content != original_content:
            ...
            print(
                f"✅ Updated {file_path.relative_to(project_root)}: {changes} replacements"
            )
            return changes
        return 0
...
def main():
    ...
    files_to_update = {
        ...
        "src/knowledge_system/processors/audio_processor.py": [
            ...
            (
                "db_service.get_video_by_file_path(",
                "db_service.get_source_by_file_path(",
            ),
        ...
        "src/knowledge_system/gui/tabs/transcription_tab.py": [
            ("video_id = ", "source_id = "),
            (
                "video_record = db_service.get_video(",
                "source_record = db_service.get_source(",
            ),
        ...
        "src/knowledge_system/processors/speaker_processor.py": [
            (
                "SELECT episode_id FROM episodes WHERE video_id",
                "SELECT source_id FROM segments WHERE source_id",
            ),
        ],
    }
