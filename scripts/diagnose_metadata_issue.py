...
def diagnose(video_id_or_url: str):
    ...
    if not video:
        ...
        print("  2. Or paste the YouTube URL in the Transcription tab (it will download automatically)")
        print()
        print("The video must be downloaded through the app to save metadata to database.")
        return
    ...
    for field_name, field_value in checks.items():
        if field_value:
            if field_name == "Description":
                preview = str(field_value)[:80] + "..." if len(str(field_value)) > 80 else str(field_value)
    ...
    if video.platform_tags:
        tags = [tag_assoc.tag.tag_name for tag_assoc in video.platform_tags if tag_assoc.tag]
    ...
    if video.platform_categories:
        categories = [cat_assoc.category.category_name for cat_assoc in video.platform_categories if cat_assoc.category]
    ...
    if video.thumbnail_local_path:
        ...
        if thumb_path.exists():
            ...
            print(f"⚠️  Thumbnail path in database but file doesn't exist: {thumb_path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print('  python scripts/diagnose_metadata_issue.py "https://www.youtube.com/watch?v=dQw4w9WgXcQ"')
        sys.exit(1)
