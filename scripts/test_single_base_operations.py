...
from src.knowledge_system.database.hce_operations import store_mining_results, load_mining_results
from src.knowledge_system.processors.hce.unified_miner import UnifiedMinerOutput
from src.knowledge_system.database.models import MediaSource, Episode, Claim
...
def test_cross_base_foreign_keys():
    ...
    output = UnifiedMinerOutput({
        'claims': [
            {'claim_id': 'claim1', 'claim_text': 'Test claim', 'claim_type': 'factual'},
            {'claim_id': 'claim2', 'claim_text': 'Another claim', 'claim_type': 'forecast'}
        ],
        'jargon': [
            {'term_id': 'term1', 'term': 'API', 'definition': 'Application Programming Interface', 'category': 'technical'}
        ],
        'people': [
            {'person_id': 'person1', 'name': 'John Doe', 'role': 'Expert'}
        ],
        'mental_models': [
            {'concept_id': 'concept1', 'concept_name': 'Systems Thinking', 'description': 'Holistic approach'}
        ]
    })
    ...
    with db.get_session() as session:
        ...
        if len(all_claims) != 2:
            print(f"   ✗ Expected 2 claims through relationship, found {len(all_claims)}")
            return False
        
        print(f"   ✓ Successfully queried {len(all_claims)} claims through MediaSource → Episode → Claim")
    ...
    print("  • Bidirectional relationships work (episode.media_source, media_source.episodes)")
