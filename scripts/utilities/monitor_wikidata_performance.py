    ...
    try:
        ...
        with db.get_session() as session:
            ...
            high_conf = session.execute(
                text("SELECT COUNT(*) FROM claim_categories WHERE match_confidence = 'high'")
            ).scalar()
            medium_conf = session.execute(
                text("SELECT COUNT(*) FROM claim_categories WHERE match_confidence = 'medium'")
            ).scalar()
            low_conf = session.execute(
                text("SELECT COUNT(*) FROM claim_categories WHERE match_confidence = 'low'")
            ).scalar()
            
            print(f"\nConfidence Distribution:")
            print(f"  High (auto-accepted):  {high_conf:4d} ({high_conf/total_categorizations*100:5.1f}%)")
            print(f"  Medium (reviewed):     {medium_conf:4d} ({medium_conf/total_categorizations*100:5.1f}%)")
            print(f"  Low (vocab gaps):      {low_conf:4d} ({low_conf/total_categorizations*100:5.1f}%)")
            ...
            for i, (name, qid, count, avg_rel) in enumerate(top_categories, 1):
                print(f"  {i:2d}. {name} ({qid}) - {count} uses, {avg_rel:.2f} avg relevance")
            ...
            vocab_gap_rate = low_conf / total_categorizations if total_categorizations > 0 else 0
            
            if vocab_gap_rate > 0.20:
                print(f"⚠️  VOCABULARY EXPANSION NEEDED")
                print(f"   Vocab gap rate: {vocab_gap_rate:.1%} (threshold: 20%)")
                print(f"   Action: Add more WikiData categories to cover content topics")
                
                # Suggest categories to add
                print(f"\n   Categories frequently causing low confidence:")
                low_conf_cats = session.execute(text("""
                    SELECT 
                        cc.freeform_input,
                        COUNT(*) as frequency
                    FROM claim_categories cc
                ...
                if low_conf_cats:
                    for freeform, freq in low_conf_cats:
                        print(f"      • '{freeform}' ({freq} times) - consider adding to vocabulary")
            
            # Check review rate
            review_rate = medium_conf / total_categorizations if total_categorizations > 0 else 0
            ...
            auto_accept_rate = high_conf / total_categorizations if total_categorizations > 0 else 0
            ...
            used_categories = session.execute(text("""
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            for freeform, closest_id, closest_name, rel_score, freq in suggestions:
                ...
                print(f"     Currently maps to: {closest_name} ({closest_id}) at {rel_score:.2f}")
