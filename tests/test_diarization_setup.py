...
TEST_OUTPUTS_DIR = PROJECT_ROOT / "data" / "test_files" / "Test Outputs" / "diarization_test"
...
def test_diarization_transcription():
    ...
    cmd = CLI_CMD + [
        "transcribe",
        "--input", str(TEST_INPUTS_DIR / "harvard.wav"),
        "--output", str(TEST_OUTPUTS_DIR),
        "--model", "base",
        "--format", "vtt",
        ...
        if vtt_file.exists():
            ...
            with open(vtt_file, 'r') as f:
                ...
                if "<v Speaker_" in content:
                    ...
                    print("⚠️  No speaker labels detected - may have fallen back to basic transcription")
                    return False
        else:
...
def show_setup_instructions():
    ...
    print("   b) Accept license at https://huggingface.co/pyannote/speaker-diarization-3.1")
