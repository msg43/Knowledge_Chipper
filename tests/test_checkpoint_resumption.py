...
def test_checkpoint_overwrite(orchestrator):
    ...
    job_id = orchestrator.create_job(
        job_type="mine", input_id="test_episode_456", config={}, auto_process=False
    )

    run_id = orchestrator.create_job_run(job_id)

    # Save initial checkpoint
    orchestrator.save_checkpoint(run_id, {"stage": "parsing", "total_segments": 50})
    ...
    orchestrator.save_checkpoint(
        run_id,
        {"stage": "mining", "total_segments": 50, "completed_segments": ["seg_0001"]},
    )
...
def test_no_checkpoint_returns_none(orchestrator):
    ...
    job_id = orchestrator.create_job(
        job_type="mine", input_id="test_episode_789", config={}, auto_process=False
    )
...
def test_checkpoint_with_completed_stage(orchestrator):
    ...
    job_id = orchestrator.create_job(
        job_type="mine", input_id="test_episode_complete", config={}, auto_process=False
    )
    ...
    final_result = {
        ...
        "result": {"claims_extracted": 42, "evidence_spans": 100},
    }
...
def test_checkpoint_with_error_stage(orchestrator):
    ...
    orchestrator.save_checkpoint(
        run_id, {"stage": "failed", "error": "File not found: /tmp/missing.mp3"}
    )
...
def test_multiple_runs_different_checkpoints(orchestrator):
    ...
    job_id = orchestrator.create_job(
        job_type="mine", input_id="test_episode_multi", config={}, auto_process=False
    )

    # Create first run and save checkpoint
    run_id_1 = orchestrator.create_job_run(job_id)
    orchestrator.save_checkpoint(
        run_id_1, {"stage": "mining", "completed_segments": ["seg_0001"]}
    )

    # Create second run and save different checkpoint
    run_id_2 = orchestrator.create_job_run(job_id)
    orchestrator.save_checkpoint(
        run_id_2, {"stage": "mining", "completed_segments": ["seg_0001", "seg_0002"]}
    )
