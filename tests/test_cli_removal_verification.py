...
class TestCLIRemovalVerification:
    ...
    def test_pyproject_has_gui_only_entry_points(self):
        ...
        assert (
            'knowledge-chipper = "knowledge_system.gui:main"' in content
        ), "Missing GUI entry point 'knowledge-chipper'"
        assert (
            'kc = "knowledge_system.gui:main"' in content
        ), "Missing GUI entry point 'kc'"

        # Should NOT have old CLI entry points
        assert (
            'knowledge-system = "knowledge_system.cli:main"' not in content
        ), "Old CLI entry point still exists"
        assert (
            "knowledge_system.commands" not in content
    ...
    def test_processors_init_exports_no_cli_classes(self):
        ...
        assert (
            "SummarizerProcessor removed" in content
            or "CLI removed" in content
            or "System2Orchestrator" in content
