...
def get_transcribe_tab(main_window: QObject) -> Optional[QWidget]:
...
def get_summarize_tab(main_window: QObject) -> Optional[QWidget]:
...
def find_combo_by_label(parent: QWidget, label_text: str) -> Optional[QComboBox]:
    ...
    for combo in parent.findChildren(QComboBox):
        ...
        if combo_parent:
            ...
            if hasattr(combo_parent, 'title') and label_text.lower() in combo_parent.title().lower():
                return combo
    return None


def find_checkbox_by_label(parent: QWidget, label_text: str) -> Optional[QCheckBox]:
...
def set_language(tab: QWidget, language: str) -> bool:
    ...
    if not lang_combo:
        for combo in tab.findChildren(QComboBox):
            if 'language' in combo.objectName().lower() or 'lang' in combo.objectName().lower():
                lang_combo = combo
                break
...
def wait_for_completion(tab: QWidget, timeout_seconds: int = 120, check_interval: float = 1.0) -> bool:
    """
...
def is_processing_complete(tab: QWidget) -> bool:
    ...
    for progress_bar in tab.findChildren(QProgressBar):
        if progress_bar.maximum() > 0 and progress_bar.value() >= progress_bar.maximum():
            return True
    ...
    for label in tab.findChildren(QLabel):
        ...
        if any(word in label_text for word in ['complete', 'finished', 'done', 'success']):
            return True
...
def check_whisper_cpp_installed() -> bool:
    ...
    import shutil
    return shutil.which("whisper-cli") is not None or shutil.which("whisper") is not None
