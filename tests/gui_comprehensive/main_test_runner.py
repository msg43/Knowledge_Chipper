            ...
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    if (proc.info['pid'] != current_pid and 
                        'knowledge' in proc.info['name'].lower()):
                        proc.terminate()
...
Examples:
  ...
  python -m tests.gui_comprehensive.main_test_runner all --config comprehensive_config.yaml
    ...
    parser.add_argument(
        "--timeout",
        type=int,
        help="Override default timeout per test (seconds)"
    )
...
def print_results_summary(results_list):
    ...
    if total_failed == 0:
        ...
        print(f"⚠️  {total_failed} tests failed. Check the detailed reports for issues.")
