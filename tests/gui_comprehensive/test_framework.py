...
class GUITestFramework:
    ...
    def __init__(self, app: QApplication, timeout: int = 300):
        ...
        self.current_test_suite: Optional[TestSuiteResults] = None
    ...
    def _find_existing_gui_window(self) -> Optional[Any]:
            ...
            for widget in QApplication.allWidgets():
                if hasattr(widget, 'windowTitle') and widget.windowTitle():
                    ...
                    if "Knowledge" in title or "Chipper" in title:
                        logger.info(f"Found potential Knowledge Chipper window: {title}")
                        # Check if it has tabs (QTabWidget)
                        if hasattr(widget, 'tabs') or widget.findChild(QApplication.allWidgets()[0].__class__.__bases__[0], 'tabs'):
                            return widget
                        # Check for QTabWidget child
                        tab_widgets = widget.findChildren(QApplication.allWidgets()[0].__class__.__bases__[0])
    ...
    def setup_main_window(self) -> MainWindow:
        ...
        try:
            from knowledge_system.gui.main_window_pyqt6 import MainWindow as RealMainWindow
            ...
            try:
                ...
                if thread_pool:
                    ...
                    if thread_pool.activeThreadCount() > 0:
                        logger.warning(f"Force-terminating {thread_pool.activeThreadCount()} remaining threads")
    ...
    def finish_test_suite(self) -> Optional[TestSuiteResults]:
        """
        ...
        logger.info(f"Results: {self.current_test_suite.passed_tests}/{self.current_test_suite.total_tests} passed "
    ...
    def run_test(self, test_name: str, test_func: Callable[[], TestResult]) -> TestResult:
        """
        ...
        try:
            ...
            if result.success:
                ...
                error_summary = '; '.join(result.errors) if result.errors else "No specific errors recorded"
                logger.warning(f"Test '{test_name}' FAILED in {result.duration}: {error_summary}")
                
                # Add more detailed logging for empty failures
                if not result.errors:
                    logger.warning(f"Test failed with no errors - validation metadata: {result.metadata.get('validation_results', 'none')}")
    ...
    def navigate_to_tab(self, tab_name: str, timeout: Optional[int] = None) -> bool:
        """
        ...
        try:
            ...
            while tab_widget.currentIndex() != tab_index:
                if time.time() - start_time > timeout:
                    logger.error(f"Timeout waiting for tab '{tab_name}' to become active")
                    return False
                self._process_events()
                time.sleep(0.1)
    ...
    def load_files(self, file_paths: List[Path], tab_name: Optional[str] = None) -> bool:
        """
