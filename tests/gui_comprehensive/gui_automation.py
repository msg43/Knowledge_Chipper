...
from PyQt6.QtWidgets import (
    QWidget, QTabWidget, QPushButton, QCheckBox, QLineEdit, 
    QListWidget, QFileDialog, QGroupBox, QLabel, QApplication
)
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                try:
                    if (proc.info['pid'] != current_pid and 
                        any(name in proc.info['name'].lower() for name in process_names)):
                        proc.terminate()
                        killed_count += 1
...
class GUIAutomation:
    ...
    def click_button(self, button_text: str, parent: Optional[QWidget] = None) -> bool:
        """
        ...
        try:
            ...
            for i, button in enumerate(buttons):
                logger.debug(f"  Button {i}: text='{button.text()}', enabled={button.isEnabled()}, visible={button.isVisible()}")
            
            # Look for exact text match first
            for button in buttons:
                if button.text() == button_text and button.isEnabled() and button.isVisible():
            ...
            for button in buttons:
                if button_text.lower() in button.text().lower() and button.isEnabled() and button.isVisible():
                    logger.info(f"Clicking button (partial match): '{button.text()}' for '{button_text}'")
                    QTest.mouseClick(button, Qt.MouseButton.LeftButton)
                    QTest.qWait(self.default_delay)
                    return True
    ...
    def get_available_buttons(self) -> List[str]:
        ...
        try:
            ...
            return [btn.text() for btn in buttons if btn.isVisible() and btn.text().strip()]
            
        except Exception as e:
            logger.error(f"Error getting available buttons: {e}")
            return []
    
    def find_button(self, button_text: str, parent: Optional[QWidget] = None) -> Optional[QPushButton]:
        """
    ...
    def set_checkbox(self, checkbox_text: str, checked: bool, 
        ...
        try:
            ...
            for i, checkbox in enumerate(checkboxes):
                logger.debug(f"  Checkbox {i}: text='{checkbox.text()}', enabled={checkbox.isEnabled()}, visible={checkbox.isVisible()}")
            
            # Look for exact text match first
            for checkbox in checkboxes:
                if checkbox.text() == checkbox_text and checkbox.isEnabled() and checkbox.isVisible():
            ...
            # If no exact match, try partial match
            for checkbox in checkboxes:
                if checkbox_text.lower() in checkbox.text().lower() and checkbox.isEnabled() and checkbox.isVisible():
                    if checkbox.isChecked() != checked:
                        logger.info(f"Setting checkbox (partial match): '{checkbox.text()}' to {checked} for '{checkbox_text}'")
                        QTest.mouseClick(checkbox, Qt.MouseButton.LeftButton)
                        QTest.qWait(self.default_delay)
                    return True
    ...
    def set_text_field(self, field_name: str, text: str, 
                      parent: Optional[QWidget] = None) -> bool:
        """
        ...
        try:
            ...
            for field in text_fields:
                if (field.objectName() == field_name or 
                    field.placeholderText() == field_name) and field.isEnabled():
    ...
    def select_tab(self, tab_name: str) -> bool:
        ...
        try:
            ...
            for tab_widget in tab_widgets:
                for i in range(tab_widget.count()):
                    ...
                    if tab_text == tab_name:
                        ...
                        QTest.qWait(self.default_delay * 2)  # Extra time for tab switching
                        return True
            ...
            logger.warning(f"Tab '{tab_name}' not found. Available tabs: {available_tabs}")
            return False
    ...
    def add_files_to_list(self, file_paths: List[Path], 
                         list_widget_name: str = None) -> bool:
        """
        ...
        try:
            ...
            for i, widget in enumerate(list_widgets):
                logger.info(f"  Widget {i}: objectName='{widget.objectName()}', visible={widget.isVisible()}, enabled={widget.isEnabled()}, parent={type(widget.parent()).__name__ if widget.parent() else 'None'}")
            ...
            if target_widget is None:
                logger.warning(f"File list widget not found - no widgets available (had {len(list_widgets)} total widgets)")
                return False
            
            logger.info(f"✅ Successfully selected widget: objectName='{target_widget.objectName()}', visible={target_widget.isVisible()}, enabled={target_widget.isEnabled()}")
                
            logger.info(f"Adding {len(file_paths)} files to list widget: {target_widget.objectName()}")
            ...
            QTest.qWait(self.default_delay)
            logger.info(f"Successfully added {files_added}/{len(file_paths)} files to list")
        ...
        while time.time() - start_time < timeout:
            ...
            for button in buttons:
                if button.isVisible():
                    ...
                    visible_buttons.append(f"'{button_text}' ({'enabled' if button.isEnabled() else 'disabled'})")
                    current_button_state.append((button_text.lower(), button.isEnabled()))
                    ...
                    if "processing" in button_text_lower:
                        ...
                        if button.isEnabled():
                            stop_button_available = True
                    elif ("start transcription" in button_text_lower or "start processing" in button_text_lower):
                        if button.isEnabled():
            ...
            logger.debug(f"Buttons at {elapsed:.1f}s: {', '.join(visible_buttons[:3])}...")  # Limit log spam
            
            # Log detailed state every 30 seconds for long-running processes
            if elapsed > 60 and int(elapsed) % 30 == 0:
                self._log_detailed_gui_state(elapsed, visible_buttons, processing_active, start_button_ready)
            ...
            if not processing_active and start_button_ready:
                ...
                has_completion_message = any(indicator in log_output for indicator in completion_indicators)
                
                if has_completion_message:
                    logger.info("Processing completed - start button is ready again")
                    return True
                else:
                    logger.debug("Start button ready but no completion message found yet - waiting...")
            ...
            stuck_threshold = max(30, timeout * 0.2)  # At least 30 checks or 20% of timeout
            if (consecutive_no_change_count > stuck_threshold or 
                elapsed > timeout * 0.9) and not force_stop_attempted:
                ...
                has_completion = any(indicator in log_output for indicator in completion_indicators)
                has_active_processing = any(indicator in log_output[-2000:] for indicator in active_indicators)  # Check recent logs
                current_start_ready = self._is_start_button_ready()
                
                if has_completion and current_start_ready:
                    logger.info("Processing actually completed - found completion message in logs")
                    return True
                elif has_active_processing:
                    logger.info(f"Processing still active - found recent activity indicators, continuing to wait")
                    consecutive_no_change_count = 0  # Reset counter since we found activity
                    continue
                
                logger.warning(f"Processing appears stuck after {elapsed:.1f}s (no UI changes for {consecutive_no_change_count} cycles)")
            ...
            logger.debug(f"Still processing... ({elapsed:.1f}s elapsed, no-change: {consecutive_no_change_count})")
    ...
    def _attempt_force_stop(self) -> None:
        ...
        try:
            ...
            buttons = self._get_all_buttons()
            for button in buttons:
                if ("stop" in button.text().lower() and 
                    button.isVisible()):
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    ...
                    if should_kill and proc.info['pid'] != current_pid:
                        logger.warning(f"Force killing process: {proc.info['pid']} ({proc_name})")
                        try:
    ...
    def _log_detailed_gui_state(self, elapsed: float, visible_buttons: List[str], 
                               processing_active: bool, start_button_ready: bool) -> None:
        ...
        logger.info(f"Visible Buttons ({len(visible_buttons)}): {', '.join(visible_buttons)}")
    ...
    def get_error_context_for_timeout(self) -> Dict[str, Any]:
        ...
        try:
            ...
            buttons = self._get_all_buttons()
            context['gui_state'] = {
                'visible_buttons': [f"{btn.text()} ({'enabled' if btn.isEnabled() else 'disabled'})" 
                                  for btn in buttons if btn.isVisible()],
                'total_buttons': len(buttons),
                'window_title': self.main_window.windowTitle() if hasattr(self.main_window, 'windowTitle') else 'Unknown'
            }
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'cpu_percent', 'memory_info']):
                try:
                    ...
                    if any(term in proc_name or term in cmdline 
            ...
            try:
                ...
                if output_text:
                    ...
                    error_indicators = ['error', 'failed', 'exception', 'timeout', 'killed']
    ...
    def reset_gui_state(self) -> bool:
        ...
        try:
            ...
            for button in buttons:
                if ("stop" in button.text().lower() and 
                    "processing" in button.text().lower() and 
                    button.isVisible()):
            ...
            if start_button_ready:
                ...
                logger.warning(f"⚠️ Start button not ready. Available: {available_buttons}")
            ...
            for widget in app.allWidgets():
                if isinstance(widget, QPushButton) and widget.isVisible():
                    ...
                    if "Processing" in button_text and not widget.isEnabled():
                        logger.warning(f"Resetting stuck button: '{button_text}' -> enabled")
                        widget.setEnabled(True)
                        
                        # Try to restore original text based on context
                        if "transcription" in button_text.lower() or "Transcription" in button_text:
        ...
        try:
            ...
            for button in buttons:
                if button.isVisible():
                    ...
                    if ("start transcription" in button_text_lower or "start processing" in button_text_lower):
    ...
    def get_output_log_text(self) -> str:
        ...
        try:
            ...
            text_widgets = (self.main_window.findChildren(QTextEdit) + 
                          self.main_window.findChildren(QPlainTextEdit))
            
            for widget in text_widgets:
                if "output" in widget.objectName().lower() or "log" in widget.objectName().lower():
                    return widget.toPlainText()
                    
            # If no specific output widget found, return text from the largest text widget
            if text_widgets:
                return max(text_widgets, key=lambda w: len(w.toPlainText())).toPlainText()
    ...
    def find_widget_by_text(self, text: str, widget_type: type = None) -> Optional[QWidget]:
        """
