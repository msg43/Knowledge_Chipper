...
class ProcessMonitor:
    ...
    def register_process(
        ...
        logger.info(
            f"Registered process for monitoring: {name} (PID: {process.pid}, timeout: {timeout}s)"
        )
    ...
    def _check_processes(self) -> None:
        ...
        for pid, info in self.monitored_processes.items():
            ...
            if process.poll() is not None:
                ...
                logger.info(
                    f"Process {info['name']} (PID: {pid}) completed normally after {elapsed:.1f}s"
                )
            ...
            if elapsed > info["timeout"]:
                logger.warning(
                    f"Process {info['name']} (PID: {pid}) timed out after {elapsed:.1f}s"
                )
            ...
            if heartbeat_elapsed > 60:
                logger.warning(
                    f"Process {info['name']} (PID: {pid}) heartbeat timeout ({heartbeat_elapsed:.1f}s)"
                )
    ...
    def _log_process_status(self, process_info: dict[str, Any], elapsed: float) -> None:
        """Log periodic status for long-running processes."""
        try:
            cpu_avg = sum(process_info["cpu_samples"]) / len(
                process_info["cpu_samples"]
            )
            memory_current = process_info["memory_samples"][-1] / (1024 * 1024)  # MB
            memory_peak = max(process_info["memory_samples"]) / (1024 * 1024)  # MB

            logger.info(
                f"Process {process_info['name']} status at {elapsed:.0f}s: "
    ...
    def _log_process_summary(
        self, process_info: dict[str, Any], elapsed: float, reason: str
    ) -> None:
        ...
        try:
            if process_info["cpu_samples"] and process_info["memory_samples"]:
                ...
                logger.info(
                    f"=== Process Summary: {process_info['name']} ({reason}) ==="
                )
                logger.info(f"Runtime: {elapsed:.1f}s")
                logger.info(f"Command: {process_info['command']}")
                logger.info(f"CPU Usage: avg={cpu_avg:.1f}%, max={cpu_max:.1f}%")
                logger.info(
                    f"Memory: initial={memory_initial:.1f}MB, peak={memory_peak:.1f}MB"
                )
                ...
                logger.info(
                    f"Process {process_info['name']} ended ({reason}) after {elapsed:.1f}s"
                )
                logger.info(f"Command: {process_info['command']}")
...
class ProcessCleanup:
    ...
    def kill_processes_by_name(
        process_names: list[str], exclude_pids: list[int] = None
    ) -> list[int]:
        """
                ...
                if proc.info["name"].lower() == "python":
                    ...
                    if "knowledge" in cmdline or "chipper" in cmdline:
                        ...
                        if not any(exclude in cmdline for exclude in exclude_patterns):
                            logger.warning(
                                f"Killing Python Knowledge System process: {proc.info['pid']}"
                            )
                            try:
                                proc.terminate()
                                proc.wait(timeout=3)
