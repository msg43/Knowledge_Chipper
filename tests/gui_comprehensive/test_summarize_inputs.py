...
def sample_md_file():
    ...
    return Path(__file__).parent.parent / "fixtures" / "sample_files" / "sample_transcript.md"


@pytest.fixture
def sample_txt_file():
    """Path to sample text document."""
    return Path(__file__).parent.parent / "fixtures" / "sample_files" / "sample_document.txt"
...
class TestSummarizeInputs:
    ...
    def test_markdown_input(self, gui_app, test_sandbox, sample_md_file):
        ...
        if not check_ollama_running():
            pytest.fail("Ollama must be running for this test. Start with: ollama serve")
        ...
        summary = summaries[0]
        assert summary.get('llm_provider') == 'ollama', f"Expected ollama provider, got {summary.get('llm_provider')}"
        ...
        for md_file in md_files:
            if 'summary' in md_file.stem.lower() or 'summar' in md_file.parent.name.lower():
                summary_md = md_file
                break
        
        if summary_md:
            frontmatter, body = read_markdown_with_frontmatter(summary_md)
            assert 'llm_provider' in frontmatter or 'title' in frontmatter, "Markdown missing required frontmatter"
    ...
    def test_pdf_input(self, gui_app, test_sandbox):
        ...
        pdf_file = Path("/Users/matthewgreer/Projects/Knowledge_Chipper/KenRogoff_Transcript.pdf")
    ...
    def test_docx_input(self, gui_app, test_sandbox):
        ...
        docx_file = Path("/Users/matthewgreer/Projects/Knowledge_Chipper/KenRogoff_Transcript.docx")
    ...
    def test_html_input(self, gui_app, test_sandbox):
        ...
        # Add HTML file
        html_file = Path(__file__).parent.parent / "fixtures/sample_files/sample_document.html"
    ...
    def test_json_input(self, gui_app, test_sandbox):
        ...
        # Add JSON file
        json_file = Path(__file__).parent.parent / "fixtures/sample_files/sample_document.json"
    ...
    def test_rtf_input(self, gui_app, test_sandbox):
        ...
        rtf_file = Path("/Users/matthewgreer/Projects/Knowledge_Chipper/KenRogoff_Transcript.rtf")
