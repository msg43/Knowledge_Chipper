...
class TestOrchestrator:
    ...
    def __init__(self, test_data_dir: Path, output_dir: Path, 
                 auto_launch_gui: bool = True, gui_startup_timeout: int = 30):
        """
        ...
        self.framework: Optional[GUITestFramework] = None
    ...
    def launch_gui_application(self) -> bool:
        ...
        try:
            ...
            env["QT_MAC_DISABLE_FOREGROUND"] = "1"  # Prevent Qt from coming to foreground
            env["KNOWLEDGE_CHIPPER_TESTING_MODE"] = "1"  # Enable testing mode
            env["DISPLAY"] = ":0.0" if "DISPLAY" not in env else env["DISPLAY"]  # Ensure display is set
            ...
            logger.info(f"Environment KNOWLEDGE_CHIPPER_TESTING_MODE = {env.get('KNOWLEDGE_CHIPPER_TESTING_MODE', 'NOT SET')}")
            ...
            for i in range(startup_timeout):
                ...
                if self.gui_process.poll() is not None:
                    ...
                    logger.error(f"GUI process terminated unexpectedly with return code: {return_code}")
                ...
                if i % 5 == 0:
                    logger.info(f"Still waiting for GUI startup... ({i+1}/{startup_timeout}s)")
        ...
        if self.gui_process and not self.gui_launched_externally:
            try:
                ...
                try:
                    ...
                    logger.warning("GUI process didn't terminate gracefully, forcing kill")
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            if self.auto_launch_gui:
                ...
                logger.info("GUI auto-launch disabled - assuming GUI is already running")
    ...
    def _get_quick_test_cases(self) -> list[dict]:
        ...
        doc_files = list((self.test_data_dir / "sample_files" / "documents").glob("*.txt"))
        if doc_files:
            quick_test_cases.append({
    ...
    def run_comprehensive_tests(self) -> TestSuiteResults:
        ...
        for i, test_case in enumerate(self.test_matrix):
            ...
            if not result.success:
                ...
                logger.warning(f"Test failed: {consecutive_failures} consecutive failures")
                
                if consecutive_failures >= max_consecutive_failures:
                    logger.error(f"Circuit breaker activated: {consecutive_failures} consecutive failures. Stopping test suite to prevent infinite hangs.")
                    break
            else:
    ...
    def run_stress_tests(self) -> TestSuiteResults:
        ...
        for test_case in stress_test_cases:
            ...
            def run_test():
                return self._run_single_test(test_case, quick_mode=False, stress_mode=True)
    ...
    def _run_single_test(self, test_case: Dict[str, Any], quick_mode: bool = False, 
                        stress_mode: bool = False) -> TestResult:
        """
        ...
        try:
            ...
            if not button_clicked:
                ...
                result.errors.append(f"Failed to start processing - no valid start button found. Available: {available_buttons}")
                result.success = False  # Explicitly mark as failed
                logger.warning(f"Test failed: Cannot start processing in tab '{current_tab}'. Available buttons: {available_buttons}")
                return result
            ...
            logger.info(f"Starting processing with {timeout}s timeout for {test_case.get('operation', 'unknown')} operation")
            ...
            if not result.success:
                if result.errors:
                    logger.warning(f"Test failed with {len(result.errors)} errors: {'; '.join(result.errors)}")
                if validation_failures:
                    validation_error_details = []
                    for v in validation_failures:
                        validation_error_details.extend(v.errors)
                    logger.warning(f"Test failed validation with {len(validation_failures)} validation failures: {'; '.join(validation_error_details)}")
    ...
    def _validate_test_outputs(self, test_case: Dict[str, Any], 
                             test_result: TestResult) -> List[ValidationResult]:
        ...
        try:
            ...
            output_directories = [
                ...
                Path("/Users/matthewgreer/Projects/SAMPLE OUTPUTS"),  # Default app output
            ...
            if expected_files:
                for output_dir in output_directories:
                    ...
                    for file_name in expected_files:
                        ...
                        if file_path.exists():
                            ...
                            # Validate content
                            content_result = self._validate_file_content(file_path, operation)
                ...
                existence_result = ValidationResult(
                    ...
                    score=len(files_found) / len(expected_files) if expected_files else 1.0,
                    errors=[] if len(files_found) >= len(expected_files) else [f"Missing output files: {set(expected_files) - {f.name for f in files_found}}"],
                    warnings=[],
                    metadata={'expected_files': expected_files, 'found_files': [str(f) for f in files_found]}
                )
    ...
    def _check_gui_success_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_count = sum(1 for indicator in success_indicators if indicator in log_output)
            error_count = sum(1 for indicator in error_indicators if indicator in log_output)
            ...
            if start_button_ready and success_count > 0 and error_count == 0:
                ...
                indicators['warnings'].append(f"Some errors detected but processing completed ({success_count} success, {error_count} errors)")
            elif start_button_ready:
                indicators['completed_successfully'] = False
                indicators['success_score'] = 0.3
                indicators['errors'].append("Processing completed but no clear success indicators found")
            else:
                indicators['completed_successfully'] = False
                indicators['success_score'] = 0.0
                indicators['errors'].append("Start button not ready - processing may not have completed")
    ...
    def _validate_file_content(self, file_path: Path, operation: str) -> 'ValidationResult':
        ...
        try:
            ...
            if len(content) < 10:
                return ValidationResult(
                    is_valid=False,
                    score=0.0,
                    errors=[f"File {file_path.name} is too short ({len(content)} characters)"],
                    warnings=[],
                    metadata={'file_size': len(content)}
                )
            
            # Check for error indicators in content
            error_indicators = ["[Error]", "[Failed]", "transcription failed", "processing failed"]
            errors_found = [indicator for indicator in error_indicators if indicator.lower() in content.lower()]
            
            if errors_found:
                return ValidationResult(
                    is_valid=False,
                    score=0.2,
                    errors=[f"Error indicators found in {file_path.name}: {errors_found}"],
                    warnings=[],
                    metadata={'content_length': len(content), 'errors_found': errors_found}
                )
            ...
            # Calculate score
            if success_count >= len(success_indicators) * 0.7:  # 70% of expected indicators
                score = 1.0
                is_valid = True
                errors = []
                ...
                is_valid = True
                errors = []
                warnings = [f"Only found {success_count}/{len(success_indicators)} expected content indicators"]
            else:
                score = 0.4
    ...
    def _configure_operation_settings(self, config: dict) -> None:
        ...
        try:
            ...
            for checkbox_name, should_be_checked in checkboxes.items():
                logger.debug(f"Setting checkbox '{checkbox_name}' to {should_be_checked}")
                success = self.automation.set_checkbox(checkbox_name, should_be_checked)
                if not success:
                    logger.warning(f"Failed to set checkbox '{checkbox_name}' to {should_be_checked}")
    ...
    def _get_smoke_test_cases(self) -> List[Dict[str, Any]]:
        ...
        for test_case in self.test_matrix:
            ...
            if combination not in seen_combinations:
                ...
                smoke_test_case['config']['checkboxes'] = smoke_test_case['config'].get('checkboxes', {}).copy()
                smoke_test_case['config']['checkboxes']['Enable speaker diarization'] = False
                
                logger.info(f"🧪 Smoke test: Disabling diarization for {test_case['file_type']} / {test_case['tab']}")
    ...
    def _generate_final_report(self) -> None:
        ...
        try:
            report = {
                "test_run": {
                    "start_time": self.overall_start_time.isoformat() if self.overall_start_time else None,
    ...
    def _perform_aggressive_cleanup(self) -> None:
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'ppid']):
                try:
                    ...
                    kill_indicators = [
                        ...
                        'knowledge_system', 'transcribe', 'python'
                    ]
                    
                    should_kill = False
                    for indicator in kill_indicators:
                        if (indicator in proc_name or 
                            indicator in cmdline or
                            'knowledge' in cmdline or
                            'chipper' in cmdline):
                            
                            # Don't kill GUI processes or our parent
                            if ('gui' not in cmdline and 
                                'test' not in cmdline and
                                proc.info['pid'] != os.getppid()):
                                should_kill = True
                                break
                    
                    if should_kill:
                        logger.warning(f"Killing stuck process: {proc.info['pid']} ({proc_name})")
                        try:
                            proc.terminate()
                            proc.wait(timeout=3)
            ...
            if killed_processes:
                logger.info(f"Killed {len(killed_processes)} stuck processes: {killed_processes}")
    ...
    def _log_timeout_analysis(self, test_case: Dict[str, Any], timeout: int, 
                             error_context: Dict[str, Any]) -> None:
        """Log comprehensive timeout analysis for debugging."""
        logger.error(f"=== TIMEOUT ANALYSIS ===")
        logger.error(f"Test: {test_case.get('file_path', 'unknown')} -> {test_case.get('operation', 'unknown')}")
        ...
        if system_state:
            logger.error(f"System: CPU={system_state.get('cpu_percent', '?')}%, "
    ...
    def _diagnose_timeout_cause(self, test_case: Dict[str, Any], 
                               error_context: Dict[str, Any]) -> None:
        ...
        knowledge_processes = error_context.get('process_info', {}).get('knowledge_processes', [])
        stuck_processes = [p for p in knowledge_processes if p['cpu_percent'] < 1]
        if stuck_processes:
            logger.error(f"🚫 {len(stuck_processes)} processes with low CPU usage (possibly stuck)")
        
        high_memory_processes = [p for p in knowledge_processes if p['memory_mb'] > 1000]
        ...
        if file_path and hasattr(file_path, 'stat'):
            try:
                ...
                if 'transcribe' in operation and file_size_mb > 100:
                    logger.error(f"📁 Large file ({file_size_mb:.1f}MB) for transcription - may need longer timeout")
            except:
                pass
