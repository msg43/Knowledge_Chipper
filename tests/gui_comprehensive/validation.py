...
class OutputValidator:
    ...
    def validate_file_format(self, file_path: Path, expected_type: str) -> ValidationResult:
        """
        ...
        if expected_extensions and file_path.suffix.lower() not in expected_extensions:
            errors.append(f"Unexpected file extension: {file_path.suffix}, expected one of {expected_extensions}")
            score -= 0.3
        ...
        if file_size_mb < min_size:
            errors.append(f"File too small: {file_size_mb:.2f}MB, minimum: {min_size}MB")
            score -= 0.2
        elif file_size_mb > max_size:
            warnings.append(f"File very large: {file_size_mb:.2f}MB, maximum recommended: {max_size}MB")
            score -= 0.1
    ...
    def validate_processing_time(self, start_time: datetime, end_time: datetime,
                               file_size_mb: float, operation_type: str) -> ValidationResult:
        """
        ...
        if duration_seconds > expected_time * 2:
            errors.append(f"Processing too slow: {duration_seconds:.1f}s, expected max: {expected_time * 2:.1f}s")
            score = 0.3
        elif duration_seconds > expected_time * 1.5:
            warnings.append(f"Processing slower than expected: {duration_seconds:.1f}s, expected: {expected_time:.1f}s")
            score = 0.7
        ...
        metadata = {
            ...
            "performance_ratio": duration_seconds / expected_time if expected_time > 0 else float('inf')
        }
    ...
    def _validate_text_content(self, content: str, content_type: str) -> ValidationResult:
        """
        ...
        if len(content) < min_length:
            errors.append(f"Content too short: {len(content)} characters, minimum: {min_length}")
            score -= 0.4
            
        # Check maximum length
        max_length = rules.get("max_length", float('inf'))
        if len(content) > max_length:
            warnings.append(f"Content very long: {len(content)} characters, maximum recommended: {max_length}")
            score -= 0.1
    ...
    def generate_validation_report(self, results: List[ValidationResult], 
        ...
        average_score = sum(r.score for r in results) / total_results if total_results > 0 else 0.0
        ...
        report = {
            ...
            "summary": {
                ...
                "success_rate": (valid_results / total_results * 100) if total_results > 0 else 0.0,
