...
class TestOrchestratorIntegration:
    ...
    async def test_checkpoint_resume_after_failure(
        self, test_db_service, sample_transcript
    ):
        ...
        orchestrator.save_checkpoint(run_id, {"last_segment": 1, "partial_results": 1})
        ...
        with test_db_service.get_session() as session:
            ...
            for response in responses:
                request = (
                    ...
                    .filter_by(request_id=response.request_id)
                    .first()
                )
...
class TestJobManagement:
    ...
    def test_create_multiple_jobs(self, test_db_service):
        ...
        for i in range(3):
            job_id = orchestrator.create_job(
                job_type="mine", input_id=f"episode_{i}", config={"test": i}
            )
    ...
    async def test_list_jobs(self, test_db_service):
        ...
        # Create a few jobs
        for i in range(3):
            orchestrator.create_job(job_type="mine", input_id=f"episode_{i}", config={})
    ...
    async def test_job_run_status_transitions(self, test_db_service):
        ...
        # Create job
        job_id = orchestrator.create_job(
            job_type="mine", input_id="test_episode", config={}
        )
        ...
        orchestrator.update_job_run_status(
            run_id, "succeeded", metrics={"processed": 10}
        )
