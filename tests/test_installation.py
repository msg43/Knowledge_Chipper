...
def test_package_builds():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "build", "--wheel"], capture_output=True, text=True
    )
...
def test_cli_help():
    ...
    result = subprocess.run(
        ["knowledge-system", "--help"], capture_output=True, text=True
    )
    assert result.returncode == 0
...
def test_version_command():
    ...
    result = subprocess.run(
        ["knowledge-system", "--version"], capture_output=True, text=True
    )
    assert result.returncode == 0
    assert "Knowledge System" in result.stdout
...
def test_fresh_install():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Install the package
        result = subprocess.run(
            [str(pip_exe), "install", "-e", "."], capture_output=True, text=True
        )
