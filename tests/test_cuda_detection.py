...
class TestCUDADetection:
        ...
        with patch('knowledge_system.utils.hardware_detection.TORCH_AVAILABLE', True):
            with patch('torch.cuda.is_available', return_value=True):
                with patch('torch.cuda.device_count', return_value=1):
                    with patch('torch.cuda.get_device_name', return_value='GeForce RTX 3080'):
        ...
        with patch('knowledge_system.utils.hardware_detection.TORCH_AVAILABLE', True):
            with patch('torch.cuda.is_available', return_value=True):
                with patch('torch.cuda.device_count', return_value=2):
                    with patch('torch.cuda.get_device_name', side_effect=['RTX 4090', 'RTX 4090']):
                        with patch('torch.cuda.get_device_properties', return_value=mock_props):
    ...
    def test_performance_characteristics_cuda_optimization(self):
        ...
        max_concurrent, optimal_batch, recommended_model, recommended_device = detector._calculate_performance_characteristics(
...
class TestCUDAIntegration:
    ...
    def test_non_apple_silicon_detection(self, mock_machine, mock_system):
        ...
        with patch.object(detector, '_detect_gpu_acceleration', return_value=(ChipType.NVIDIA_CUDA, GPUType.NVIDIA_CUDA, mock_cuda_specs)):
