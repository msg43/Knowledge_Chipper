...
class BugDetector:
    ...
    def _detect_assertion_failures(self, content: str) -> List[BugReport]:
        ...
        for match in matches:
            ...
            bug = BugReport(
                ...
                stack_traces=self._extract_stack_traces(context),
                affected_components=self._identify_components(context),
                test_cases_failed=self._extract_test_names(content[max(0, match.start()-200):match.start()]),
                first_seen=datetime.now(),
            )
            bugs.append(bug)
    ...
    def generate_bug_report_file(self, output_dir: Path) -> Path:
        ...
        report_path = output_dir / f"bug_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        with open(report_path, 'w') as f:
            ...
            for severity in ["critical", "high", "medium", "low"]:
                ...
                for bug in bugs:
                    ...
                    f.write(f"- **First Seen**: {bug.first_seen.strftime('%Y-%m-%d %H:%M:%S')}\n")
                    f.write(f"- **Affected Components**: {', '.join(bug.affected_components)}\n\n")
                    ...
                    if bug.test_cases_failed:
                        f.write(f"**Failed Tests**: {', '.join(bug.test_cases_failed[:5])}\n\n")
                    
                    f.write("---\n\n")
        
        return report_path
    
    def analyze_all_reports(self, reports_dir: Optional[Path] = None) -> Path:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Automated bug detection from test results")
    parser.add_argument(
        "reports_dir",
        type=Path,
