...
class CoverageAnalyzer:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if any(keyword in node.name for keyword in ["Tab", "Widget", "Dialog", "Window"]):
                        return node.name
    ...
    def _analyze_class_file(self, file_path: Path, class_name: str, component_type: str) -> ComponentCoverage:
    ...
    def _find_tested_workflow_steps(self, workflow_name: str, steps: List[str]) -> List[str]:
    ...
    def generate_coverage_report(self, output_file: Path) -> None:
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"- **Total Test Functions**: {test_stats['total_test_functions']}\n")
            f.write(f"- **GUI Test Files**: {test_stats['gui_test_files']}\n")
            f.write(f"- **Unit Test Files**: {test_stats['unit_test_files']}\n")
            f.write(f"- **Integration Test Files**: {test_stats['integration_test_files']}\n\n")
            
            # GUI Component Coverage
            f.write("## GUI Component Coverage\n\n")
            
            if self.gui_components:
                total_methods = sum(c.total_methods for c in self.gui_components.values())
                tested_methods = sum(c.tested_methods for c in self.gui_components.values())
                overall_coverage = (tested_methods / total_methods * 100) if total_methods > 0 else 0
                
                f.write(f"**Overall Coverage**: {overall_coverage:.1f}% ({tested_methods}/{total_methods} methods)\n\n")
                
                f.write("| Component | Type | Coverage | Tested | Total |\n")
                f.write("|-----------|------|----------|--------|-------|\n")
                
                for component in sorted(self.gui_components.values(), key=lambda c: c.coverage_percentage, reverse=True):
                    f.write(f"| {component.component_name} | {component.component_type} | "
                           f"{component.coverage_percentage:.1f}% | {component.tested_methods} | {component.total_methods} |\n")
                
                f.write("\n### Components Needing More Tests\n\n")
                low_coverage = [c for c in self.gui_components.values() if c.coverage_percentage < 50]
                
                if low_coverage:
                    for component in sorted(low_coverage, key=lambda c: c.coverage_percentage):
                        f.write(f"#### {component.component_name} ({component.coverage_percentage:.1f}% coverage)\n\n")
                        if component.untested_method_names:
                            f.write("Untested methods:\n")
                            for method in component.untested_method_names[:10]:  # Limit to 10
                                f.write(f"- `{method}()`\n")
                            if len(component.untested_method_names) > 10:
                                f.write(f"- ... and {len(component.untested_method_names) - 10} more\n")
                        f.write("\n")
                else:
            ...
            if self.workflows:
                ...
                overall_workflow_coverage = (tested_steps / total_steps * 100) if total_steps > 0 else 0
                
                f.write(f"**Overall Workflow Coverage**: {overall_workflow_coverage:.1f}% ({tested_steps}/{total_steps} steps)\n\n")
                
                f.write("| Workflow | Coverage | Tested Steps | Total Steps |\n")
                f.write("|----------|----------|--------------|-------------|\n")
                
                for workflow in sorted(self.workflows.values(), key=lambda w: w.coverage_percentage, reverse=True):
                    f.write(f"| {workflow.workflow_name} | {workflow.coverage_percentage:.1f}% | "
                           f"{workflow.steps_tested} | {workflow.steps_defined} |\n")
                
                f.write("\n### Workflows Needing More Tests\n\n")
                incomplete_workflows = [w for w in self.workflows.values() if w.coverage_percentage < 100]
                
                if incomplete_workflows:
                    for workflow in sorted(incomplete_workflows, key=lambda w: w.coverage_percentage):
                        f.write(f"#### {workflow.workflow_name} ({workflow.coverage_percentage:.1f}% coverage)\n\n")
            ...
            if self.gui_components:
                low_coverage_components = [c for c in self.gui_components.values() if c.coverage_percentage < 50]
                ...
                medium_coverage_components = [c for c in self.gui_components.values() if 50 <= c.coverage_percentage < 80]
            ...
            if recommendations:
                ...
                f.write("✅ **Excellent!** Test coverage looks good. Keep maintaining and expanding tests.\n\n")
            ...
            if self.gui_components:
                ...
                if overall_coverage >= 80:
                    ...
                    f.write("🟡 **Good coverage** - Most GUI components are tested, but some gaps remain.\n\n")
                elif overall_coverage >= 40:
                    f.write("🟠 **Moderate coverage** - Many GUI components need more tests.\n\n")
                else:
                    f.write("🔴 **Low coverage** - Significant testing gaps. Add more comprehensive tests.\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze test coverage for GUI components and workflows")
    parser.add_argument(
        "--src-dir",
        type=Path,
