            ...
            if timestamp_match:
                ...
                while i < len(lines) and lines[i].strip() and '-->' not in lines[i]:
                    ...
                    if text_line:
                        ...
                        cleaned_text = re.sub(r'<\d{2}:\d{2}:\d{2}\.\d{3}><c>', ' ', text_line)
...
def create_markdown_transcript(title, uploader, duration, upload_date, description, view_count, url, segments, output_file):
...
def run_diarization(audio_file, output_dir):
    ...
    cmd = [
        sys.executable, "-m", "knowledge_system", "transcribe",
...
def merge_diarization_with_transcript(original_segments, diarized_vtt_file, output_file):
...
Under Section 107 of the Copyright Act 1976, allowance is made for "fair use" for purposes such as criticism, commenting, news reporting, teaching, scholarship, and research."""
    ...
    # File paths
    vtt_file = test_outputs / "Jordan Peterson and Sam Harris' EPIC TOP 5 DEBATE MOMENTS ｜ Must-See Clips.en.vtt"
    audio_file = test_outputs / "Jordan Peterson and Sam Harris' EPIC TOP 5 DEBATE MOMENTS ｜ Must-See Clips.wav"
    output_file = test_outputs / "Jordan_Peterson_Sam_Harris_Debate_Moments_Diarized_Transcript.md"
    ...
    create_markdown_transcript(title, uploader, duration, upload_date, description, view_count, url, segments, output_file)
