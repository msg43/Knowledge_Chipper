...
def main() -> int:
    ...
    for file_path in required_files:
        ...
        all_passed &= print_check(
            f"File: {file_path}", full_path.exists(), f"Path: {full_path}"
        )
    ...
    for dir_path in required_dirs:
        ...
        all_passed &= print_check(
            f"Directory: {dir_path}", full_path.is_dir(), f"Path: {full_path}"
        )
    ...
    try:
        ...
        all_passed &= print_check(
            "Import knowledge_system", True, f"Version: {version}"
        )
    except ImportError as e:
        all_passed &= print_check("Import knowledge_system", False, f"Error: {e}")
    ...
    if venv_python.exists():
        try:
            ...
            all_passed &= print_check(
                "Virtual environment", result.returncode == 0, result.stdout.strip()
            )
        except Exception as e:
            all_passed &= print_check("Virtual environment", False, f"Error: {e}")
    else:
        all_passed &= print_check(
            "Virtual environment", False, "Run: python3 -m venv venv"
        )
