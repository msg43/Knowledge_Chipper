...
def benchmark_mining_with_nested_schema(
    segment_file: Path, num_segments: int = 14
) -> tuple[float, int, int]:
    """
...
def analyze_conveyor_belt(
    ...
    print(
        f"  2. Transcription: {transcription_time:>7.1f}s  ({transcription_time/60:>5.1f} min)"
    )
    print(f"  3. Mining:        {mining_time:>7.1f}s  ({mining_time/60:>5.1f} min)")
    print(f"  ----------------------------------------")
    print(
        f"  TOTAL:           {download_time + transcription_time + mining_time:>7.1f}s  ({(download_time + transcription_time + mining_time)/60:>5.1f} min)"
    )
    ...
    print(
        f"  Sequential: {sequential_total/3600:.1f} hours ({sequential_total/3600/24:.1f} days)"
    )
    print(
        f"  Parallel:   {parallel_total/3600:.1f} hours ({parallel_total/3600/24:.1f} days)"
    )
    ...
    if bottleneck[0] == "Mining":
        ...
        print(
            f"  Mining takes: {(mining_time/transcription_time)*100:.0f}% of transcription time"
        )
...
def main():
    ...
    try:
        seg_per_sec, claims, people = benchmark_mining_with_nested_schema(
            segment_file, num_segments=14
        )
    ...
    print(
        f"Optimal strategy: {'Optimize transcription' if results['bottleneck'] == 'Transcription' else 'Current mining speed is adequate' if results['bottleneck'] != 'Mining' else 'Optimize mining'}"
    )
