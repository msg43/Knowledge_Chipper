...
def main():
    """Run all tests"""
    parser = argparse.ArgumentParser(description="Test multi-account download functionality")
    parser.add_argument(
        "--cookies",
        nargs="+",
    ...
    if not args.skip_downloads and args.cookies and len(args.cookies) >= 1:
        ...
        print("⚠️ Skipping single-account download test (no cookies or --skip-downloads)\n")
    ...
    if not args.skip_downloads and args.cookies and len(args.cookies) >= 2:
        ...
        print("⚠️ Skipping multi-account download test (need 2+ cookies or --skip-downloads)\n")
